...
export function validateAddress(address: string): boolean {
    try {
        if (
            base.isHexString(address) ||
            (address.startsWith(ADDRESS_PREFIX) &&
                address.length == HEX_ADDRESS_SIZE * 2)
        ) {
...
export interface AssetTransferTransactionParams
    extends TransferTransactionParams {
    // trc10 name of assert
    assetName: string;
}

export interface TokenTransferTransactionParams
    extends TransferTransactionParams {
    // contract address
...
export function createRawTransaction(
    param: CommonTransactionParams,
    contractArray: protocol.Transaction.IContract[]
) {
...
export function getTxIdBySignedTx(signedTx: string) {
    ...
    const rawBytes = protocol.Transaction.raw
        .encode(tx.rawData as protocol.Transaction.raw)
        .finish();
...
export function transferContract(param: TransferTransactionParams) {
    // Validate addresses
    if (!validateAddress(param.toAddress)) {
        throw new Error(`Invalid toAddress: ${param.toAddress}`);
    }
...
export function assetTransfer(
    param: AssetTransferTransactionParams,
    privateKey: string
) {
...
export function getTransferData(param: TokenTransferTransactionParams) {
    const toAddress =
        '0x' + base.toHex(getAddressHash(param.toAddress)).slice(2);
    const sigId = base
        .keccak256(Buffer.from('transfer(address,uint256)'))
        .slice(0, 4);
    return Buffer.concat([
        sigId,
        abi.RawEncode(['address', 'uint256'], [toAddress, param.amount]),
    ]);
}
...
export function tokenTransferContract(param: TokenTransferTransactionParams) {
    ...
    if (!validateAddress(param.contractAddress)) {
        throw new Error(`Invalid contractAddress: ${param.contractAddress}`);
    }
...
export function tokenTransfer(
    param: TokenTransferTransactionParams,
    privateKey: string
) {
...
export function signMessage(
    ...
    if (type === 'v2') {
        ...
        let priKeyBytes = Buffer.from(base.fromHex(priKey));
        const { v, r, s } = eth.ecdsaSign(
            eth.toBuffer(messageDigest),
            priKeyBytes
        );
...
export function verifySignature(
    ...
    const messageDigest = base.keccak256(messageBytes);
    const recoveryParam =
        base.stripHexPrefix(signature).substring(128, 130) == '1c' ? 1 : 0;
...
export function getUnsignedMessage(
    type: MessageType,
    message: string,
    useTronHeader = true
): string {
...
export function getMPCSignedMessage(
    ...
    if (type === 'hex') {
        ...
        const obj = JSON.parse(message!);
        const recovery = signUtil.secp256k1.getV(
            base.fromHex(hash),
            base.toHex(r),
            base.toHex(s),
            base.fromHex(publicKey),
...
export function validSignedTransaction(tx: string, publicKey?: string) {
    ...
    if (
        publicKey &&
        !signUtil.secp256k1.verifyWithNoRecovery(
            message,
            signature,
            base.fromHex(publicKey)
        )
