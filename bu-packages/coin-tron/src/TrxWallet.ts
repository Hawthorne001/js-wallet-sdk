...
export type TrxSignParam = {
    ...
    data:
        | tron.TransferTransactionParams
        | tron.AssetTransferTransactionParams
        | tron.TokenTransferTransactionParams;
};
...
export class TrxWallet extends BaseWallet {
    ...
    async getNewAddress(param: NewAddressParams): Promise<any> {
        try {
            ...
            const publicKey = tron.getPubKeyFromPriKey(
    ...
    async signTransaction(param: SignTxParams): Promise<any> {
        try {
            ...
            const validation = await this.validAddress({
                address: txParams.data.toAddress,
            });
            ...
            if (txParams.type === 'transfer') {
                ...
                const data =
                    txParams.data as tron.AssetTransferTransactionParams;
                const tx = tron.assetTransfer(data, privateKey);
                return Promise.resolve(tx);
            } else if (txParams.type === 'tokenTransfer') {
                const data =
                    txParams.data as tron.TokenTransferTransactionParams;
    ...
    async signMessage(param: SignTxParams): Promise<string> {
        ...
        try {
            ...
            const signedMsg = tron.signMessage(
                param.data.type,
                message,
                param.privateKey,
                true
    ...
    async verifyMessage(param: VerifyMessageParams): Promise<boolean> {
        ...
        let address = param.address || '';
        return await this.ecRecover(message, signature).then(
            (recoveredAddress) => {
                return Promise.resolve(
                    address.toLowerCase() === recoveredAddress.toLowerCase()
                );
            }
        );
    }
    ...
    async getMPCTransaction(param: MpcTransactionParam): Promise<string> {
        try {
            const signedTx = tron.getMPCTransaction(
                param.raw,
    ...
    async getMPCRawMessage(param: MpcRawTransactionParam): Promise<any> {
        try {
            const msgHash = tron.getUnsignedMessage(
                param.data.type,
                param.data.message,
                true
            );
    ...
    async getHardWareMessageHash(param: SignTxParams): Promise<any> {
        try {
            const message = base.stripHexPrefix(param.data.message);
            const signedMsg = tron.getUnsignedMessage(
                param.data.type,
                message,
                true
            );
