...
describe('zkspace', () => {
    test('null privateKey should error', async () => {
        const wallet = new ZkspaceWallet();
    ...
    describe('Utility functions', () => {
        ...
        test('closestPackableTransactionAmount with large amount', () => {
            const result = closestPackableTransactionAmount(
                TEST_DATA.test.large
            );
            expect(result).toBeInstanceOf(BigNumber);
        ...
        test('closestPackableTransactionFee', () => {
            const result = closestPackableTransactionFee(
                TEST_DATA.fees.standard
            );
            expect(result).toBeInstanceOf(BigNumber);
        ...
        test('closestPackableTransactionAmount with very large value', () => {
            const result = closestPackableTransactionAmount(
                TEST_DATA.test.veryLarge
            );
            expect(result).toBeInstanceOf(BigNumber);
            expect(result.lte(BigNumber.from(TEST_DATA.test.veryLarge))).toBe(
                true
            );
        });
        ...
        test('closestPackableTransactionFee with very large value', () => {
            ...
            expect(result).toBeInstanceOf(BigNumber);
            expect(result.lte(BigNumber.from('99999999999999999999999'))).toBe(
                true
            );
        });
    });
    ...
    describe('ZkspaceWallet address validation', () => {
        let wallet: ZkspaceWallet;
