...
import { SignTxParams, InvalidPrivateKeyError } from '@okxweb3/coin-base';
import {
...
describe('zksync', () => {
    ...
    describe('ZksyncWallet address validation', () => {
        let wallet: ZksyncWallet;
        ...
        test('should accept valid Ethereum addresses', async () => {
            const validAddresses = [
                {
                    address: '0x742d35Cc6634C0532925a3b8D65428C6c2b8b5d5',
                    type: 'Standard Ethereum',
                },
                {
                    address: '0x21dceed765c30b2abea933a161479aea4702e433',
                    type: 'Lowercase',
                },
                {
                    address: '0x21DCEED765C30B2ABEA933A161479AEA4702E433',
            ...
            for (const { address, type } of validAddresses) {
                const signParams: SignTxParams = {
                    ...
                    data: {
                        ...
                        from: TEST_DATA.addresses.from,
                        to: address,
                        accountId: TEST_DATA.accounts.main,
                        nonce: TEST_DATA.nonces.alt,
                        tokenId: TEST_DATA.tokens.eth.id,
                        tokenSymbol: TEST_DATA.tokens.eth.symbol,
                        decimals: TEST_DATA.tokens.eth.decimals,
                        amounts: TEST_DATA.amounts.test.packable,
                        fees: TEST_DATA.fees.small,
                    },
                };
        ...
        test('should reject invalid recipient addresses', async () => {
            const invalidAddresses = [
                ...
                { address: '0xinvalid', reason: 'Invalid hex' },
                {
        ...
        test('should skip validation for changePubkey transactions', async () => {
            const signParams: SignTxParams = {
                privateKey: TEST_DATA.privateKeys.main,
                data: {
                    type: 'changePubkey',
