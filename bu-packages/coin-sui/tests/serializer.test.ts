    ...
    describe('getPureSerializationType', () => {
        describe('primitive types', () => {
            ...
            it('should handle all integer types', () => {
                ...
                expect(getPureSerializationType('U256', 100000000)).toBe(
                    'u256'
                );
            });

            it('should handle undefined argVal for all types', () => {
                expect(getPureSerializationType('Address', undefined)).toBe(
                    'address'
                );
                expect(getPureSerializationType('Bool', undefined)).toBe(
                    'bool'
                );
            ...
            it('should throw error for wrong type - number instead of boolean', () => {
                expect(() =>
                    getPureSerializationType('Bool', 42 as any)
                ).toThrow('Expect 42 to be boolean, received number');
            });
            ...
            it('should not throw error for integer types with wrong values - type validation is conditional', () => {
                ...
                expect(
                    getPureSerializationType('U64', 'not_a_number' as any)
                ).toBe('u64');
            });
        ...
        describe('vector types', () => {
            it('should handle Vector<U8> as string when argVal is string', () => {
                ...
                expect(getPureSerializationType(vectorU8, 'hello')).toBe(
                    'string'
                );
            });
            ...
            it('should handle Vector<U8> as string when argVal is undefined', () => {
                const vectorU8: SuiMoveNormalizedType = { Vector: 'U8' };
                expect(getPureSerializationType(vectorU8, undefined)).toBe(
                    'string'
                );
            });
            ...
            it('should handle Vector<Bool>', () => {
                ...
                expect(
                    getPureSerializationType(vectorBool, [true, false])
                ).toBe('vector<bool>');
            });

            it('should handle Vector<Address>', () => {
                const vectorAddress: SuiMoveNormalizedType = {
                    Vector: 'Address',
                };
                expect(
            ...
            it('should handle nested vectors', () => {
                ...
                expect(
                    getPureSerializationType(nestedVector, [['hello']])
                ).toBe('vector<vector<u8>>');
            });

            it('should handle empty vector', () => {
                const vectorU64: SuiMoveNormalizedType = { Vector: 'U64' };
                expect(getPureSerializationType(vectorU64, [])).toBe(
                    'vector<u64>'
                );
            });
        ...
        describe('struct types', () => {
            it('should handle ASCII String struct', () => {
                ...
                expect(getPureSerializationType(asciiString, 'hello')).toBe(
                    'string'
                );
            });
            ...
            it('should handle Sui ID struct', () => {
                ...
                expect(getPureSerializationType(suiId, '0x123')).toBe(
                    'address'
                );
            });
            ...
            it('should handle Option struct', () => {
                ...
                expect(getPureSerializationType(optionU64, [42])).toBe(
                    'vector<u64>'
                );
            });
            ...
            it('should handle nested Option<Vector<U8>>', () => {
                ...
                expect(
                    getPureSerializationType(optionVectorU8, ['hello'])
                ).toBe('vector<string>');
            });
            ...
            it('should return undefined for unknown struct', () => {
                ...
                expect(
                    getPureSerializationType(unknownStruct, 'test')
                ).toBeUndefined();
            });
        ...
        describe('complex nested types', () => {
            ...
            it('should handle Vector<Vector<U8>>', () => {
                ...
                expect(
                    getPureSerializationType(vectorVectorU8, [
                        ['hello', 'world'],
                    ])
                ).toBe('vector<vector<u8>>');
            });
            ...
            it('should handle deeply nested structures', () => {
                ...
                expect(
                    getPureSerializationType(complexType, [[[true, false]]])
                ).toBe('vector<vector<vector<bool>>>');
            });
        });

        describe('edge cases', () => {
            it('should handle null and undefined arguments gracefully', () => {
                expect(getPureSerializationType('U64', null as any)).toBe(
                    'u64'
                );
                expect(getPureSerializationType('Bool', undefined)).toBe(
                    'bool'
            ...
            it('should handle reference types', () => {
                ...
                expect(
                    getPureSerializationType(referenceType, 42)
                ).toBeUndefined();
            });
            ...
            it('should handle mutable reference types', () => {
                ...
                expect(
                    getPureSerializationType(mutableRefType, true)
                ).toBeUndefined();
            });
            ...
            it('should handle type parameters', () => {
                ...
                expect(
                    getPureSerializationType(typeParam, 'any')
                ).toBeUndefined();
            });
        });
        ...
        describe('type validation', () => {
            ...
            it('should validate boolean type with wrong type', () => {
                expect(() =>
                    ...
                    'Expect not_a_boolean to be boolean, received string'
                );
            });

            it('should validate address type with wrong type', () => {
                expect(() =>
                    getPureSerializationType('Address', 42 as any)
                ).toThrow('Expect 42 to be string, received number');
            });
            ...
            it('should handle various invalid address formats', () => {
                ...
                invalidAddresses.forEach((addr) => {
                    expect(() =>
                        getPureSerializationType('Address', addr)
                    ).toThrow('Invalid Sui Address');
                });
            });
        });
        ...
        describe('struct identification', () => {
            it('should correctly identify structs with same address, module, and name', () => {
                ...
                expect(getPureSerializationType(asciiString1, 'test')).toBe(
                    'string'
                );
                expect(getPureSerializationType(asciiString2, 'test')).toBe(
                    'string'
                );
            });
            ...
            it('should distinguish structs with different addresses', () => {
                ...
                expect(
                    getPureSerializationType(customString, 'test')
                ).toBeUndefined();
            });
