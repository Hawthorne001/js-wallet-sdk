...
describe('SuiWallet comprehensive tests', () => {
    it('null privateKey should error', async () => {
    ...
    describe('getDerivedPath', () => {
        ...
        it('should return correct derived path for different indices', async () => {
            ...
            for (const testCase of testCases) {
                const path = await wallet.getDerivedPath({
    ...
    describe('getDerivedPrivateKey', () => {
        ...
        it('should reject invalid mnemonic', async () => {
            ...
            await expect(
                wallet.getDerivedPrivateKey(param)
            ).rejects.toBeTruthy();
        });
    });
    ...
    describe('signMessage', () => {
        ...
        it('should reject empty privateKey', async () => {
            ...
            await expect(wallet.signMessage(param)).rejects.toMatch(
                /invalid private key error: cannot be empty/
            );
        });

        it('should reject non-hex privateKey', async () => {
            const message = base.toUtf8('hello world');
            const param: SignTxParams = {
        ...
        it('should reject invalid hex length privateKey', async () => {
            const message = base.toUtf8('hello world');
            // Use hex string with invalid length (e.g., 16 bytes instead of 32 bytes)
            const param: SignTxParams = {
                privateKey: '1234567890abcdef1234567890abcdef', // 16 bytes, not 32 bytes
                data: message,
            };
        ...
        it('should reject invalid Sui privateKey format', async () => {
            const message = base.toUtf8('hello world');
            // Use invalid Sui private key format
            const param: SignTxParams = {
                privateKey: 'suiprivkey1invalidformat',
                data: message,
            };
    ...
    describe('Utility functions', () => {
        ...
        describe('tryDecodeSuiPrivateKey', () => {
            ...
            it('should throw error for invalid input', () => {
                ...
                expect(() =>
                    tryDecodeSuiPrivateKey('suiprivkey1invalid')
                ).toThrow();
            });
        });
        ...
        describe('getAddressFromPrivate', () => {
            ...
            it('should handle uppercase private keys', () => {
                const result = getAddressFromPrivate(
                    validPrivateKey.toUpperCase()
                );
                expect(result.address).toMatch(/^0x[a-f0-9]{64}$/);
            });
    ...
    describe('Address Validation in Transaction Signing', () => {
        describe('paySUI transaction validation', () => {
            it('should reject empty recipient address in paySUI transactions', async () => {
                const payTransaction: PaySuiTransaction = {
                    inputCoins: [
                        {
                            objectId:
            ...
            it('should reject whitespace-only recipient address in paySUI transactions', async () => {
                const payTransaction: PaySuiTransaction = {
                    inputCoins: [
                        {
                            objectId:
            ...
            it('should reject invalid recipient address in paySUI transactions', async () => {
                const payTransaction: PaySuiTransaction = {
                    inputCoins: [
                        {
                            objectId:
            ...
            it('should reject short recipient address in paySUI transactions', async () => {
                const payTransaction: PaySuiTransaction = {
                    inputCoins: [
                        {
                            objectId:
            ...
            it('should accept valid recipient address in paySUI transactions', async () => {
                const payTransaction: PaySuiTransaction = {
                    inputCoins: [
                        {
                            objectId:
                ...
                // Should not reject due to address validation (may fail for other reasons like missing gas coins)
                try {
                    ...
                    // The failure is likely due to missing gas coins or network issues
    ...
    describe('SuiWallet validAddress method', () => {
        it('should validate correct 32-byte hex addresses', async () => {
            const result1 = await wallet.validAddress({
                address:
                    '0x1f1bfbf2d571d4f15bf7803f81e43f10bcf78ef9473ba69ff3e4664477b54c40',
            });
        ...
        it('should reject whitespace-only addresses', async () => {
            const result = await wallet.validAddress({ address: '   ' });
            expect(result.isValid).toBe(false);
        });
        ...
        it('should reject short addresses', async () => {
            const result1 = await wallet.validAddress({ address: '0x123' });
            expect(result1.isValid).toBe(false);

            const result2 = await wallet.validAddress({
                address:
                    '0x1f1bfbf2d571d4f15bf7803f81e43f10bcf78ef9473ba69ff3e4664477b54c4',
            });
            expect(result2.isValid).toBe(false);
        });

        it('should match Go implementation behavior for transaction signing', async () => {
            // Test that transaction signing uses strict validation matching Go regex
            const validWithPrefix =
                '0x1f1bfbf2d571d4f15bf7803f81e43f10bcf78ef9473ba69ff3e4664477b54c40';
            const validWithoutPrefix =
                '1f1bfbf2d571d4f15bf7803f81e43f10bcf78ef9473ba69ff3e4664477b54c40';
            const invalid63Chars =
                '1f1bfbf2d571d4f15bf7803f81e43f10bcf78ef9473ba69ff3e4664477b54c4'; // 63 chars
            const invalid65Chars =
                '0x1f1bfbf2d571d4f15bf7803f81e43f10bcf78ef9473ba69ff3e4664477b54c4'; // 65 chars
            ...
            for (const validAddress of [validWithPrefix, validWithoutPrefix]) {
                const signData = {
                    type: 'paySUI' as const,
                    data: { ...baseTransaction, recipient: validAddress },
                };

                // Should not reject due to address validation (may fail for other reasons)
                try {
                    await wallet.signTransaction({
                        privateKey: validPrivateKey,
                        data: signData,
            ...
            for (const invalidAddress of [invalid63Chars, invalid65Chars]) {
                const signData = {
                    type: 'paySUI' as const,
                    data: { ...baseTransaction, recipient: invalidAddress },
                };
        ...
        it('should reject long addresses', async () => {
            const result = await wallet.validAddress({
                address:
                    '0x1f1bfbf2d571d4f15bf7803f81e43f10bcf78ef9473ba69ff3e4664477b54c401',
            });
            expect(result.isValid).toBe(false);
        });
        ...
        it('should reject invalid hex characters', async () => {
            const result = await wallet.validAddress({
                address:
                    '0x1f1bfbf2d571d4f15bf7803f81e43f10bcf78ef9473ba69ff3e4664477b54cxxx',
            });
            expect(result.isValid).toBe(false);
        });
    });
