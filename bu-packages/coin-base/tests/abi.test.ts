...
describe('ABI Module Tests', () => {
    describe('Main ABI Function Tests', () => {
        ...
        test('ABI.stringify test', () => {
            ...
            const values = [
                123,
                Buffer.from('742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6', 'hex'),
                'hello',
            ];
    ...
    describe('Extended Bytes Tests', () => {
        ...
        test('toBuffer with negative bigint', () => {
            ...
            expect(() => toBuffer(negativeBigInt)).toThrow(
        ...
        test('bufferToInt with large number exceeding 53 bits', () => {
            ...
            expect(() => bufferToInt(largeBuffer)).toThrow(
                'Number exceeds 53 bits'
            );
        });
    ...
    describe('Extended Internal Tests', () => {
        ...
        test('stripHexPrefix with non-string input', () => {
            expect(() => stripHexPrefix(123 as any)).toThrow(
                'input must be type'
            );
        });
        ...
        test('getBinarySize with non-string input', () => {
            expect(() => getBinarySize(123 as any)).toThrow(
                'method requires input type'
            );
        });
        ...
        test('arrayContainsArray with non-array superset', () => {
            expect(() =>
                arrayContainsArray('not array' as any, [1, 2, 3])
            ).toThrow('method requires input');
        });

        test('arrayContainsArray with non-array subset', () => {
            expect(() =>
                arrayContainsArray([1, 2, 3], 'not array' as any)
            ).toThrow('method requires input');
        });
        ...
        test('getKeys with allowEmpty option', () => {
            const params = [{ key: 'value1' }, { key: '' }, { key: 'value3' }];
    ...
    describe('Extended Util Tests', () => {
        ...
        test('unpadHexString with hex string without 0x', () => {
            ...
            expect(() => unpadHexString('000123')).toThrow(
    ...
    describe('Extended Helper Tests', () => {
        test('assertIsHexString with non-hex string', () => {
            expect(() => assertIsHexString('not hex')).toThrow(
                'This method only supports 0x-prefixed hex strings'
            );
        });
        ...
        test('assertIsBuffer with non-buffer input', () => {
            expect(() => assertIsBuffer('not buffer' as any)).toThrow(
                'This method only supports Buffer'
            );
        });

        test('assertIsArray with non-array input', () => {
            expect(() => assertIsArray('not array' as any)).toThrow(
                'This method only supports number arrays'
            );
        });

        test('assertIsString with non-string input', () => {
            expect(() => assertIsString(123 as any)).toThrow(
                'This method only supports strings'
            );
        });
    });
    ...
    describe('migrate from crypto-lib', () => {
        test('bytes test', async () => {
            ...
            expect(setLengthLeft(base.fromHex('0x1234'), 1)).toEqual(
                new Buffer([0x34])
            );
            expect(setLengthRight(base.fromHex('0x1234'), 1)).toEqual(
                new Buffer([0x12])
            );
            expect(unpadBuffer(base.fromHex('0x01234'))).toEqual(
                new Buffer([0x12, 0x34])
            );
            expect(unpadArray([0, 1, 2, 3, 4, 5, 6])).toEqual([
                1, 2, 3, 4, 5, 6,
            ]);
        ...
        test('helper test', async () => {
            assertIsString('1234');
            expect(base.toHex(SoliditySHA3(['uint256'], [1234]))).toEqual(
                '17fa14b0d73aa6a26d6b8720c1c84b50984f5c188ee1c113d2361e430f1b6764'
            );
            expect(
                getKeys(
                    [
