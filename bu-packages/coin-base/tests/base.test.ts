...
import {
    ...
    checkIsDefined,
    checkIsUndefined,
} from '../src/base/precondtion';
import {
    toBigIntHex,
    fromBigIntHex,
    bigNumber2String,
    string2BigNumber,
} from '../src/base/bignumber-plus';
import {
    doubleSha256,
    hash160,
    keccak,
...
describe('Base Module Tests', () => {
    ...
    describe('Precondition Tests', () => {
        ...
        test('check with false statement and custom error', () => {
            expect(() => check(false, 'Custom error message')).toThrow(
        ...
        test('checkIsDefined with undefined value', () => {
            expect(() => checkIsDefined(undefined)).toThrow(
        ...
        test('checkIsDefined with undefined value and custom error', () => {
            expect(() =>
                checkIsDefined(undefined, 'Custom undefined error')
        ...
        test('checkIsUndefined with defined value', () => {
            expect(() => checkIsUndefined('defined value')).toThrow(
                'Expect undefined but actually defined value'
            );
        });

        test('checkIsUndefined with defined value and custom error', () => {
            expect(() =>
                checkIsUndefined('defined value', 'Custom defined error')
            ).toThrow('Custom defined error');
        });
    ...
    describe('BigNumber Plus Tests', () => {
        ...
        test('fromBigIntHex with invalid hex string', () => {
            expect(() => fromBigIntHex('invalid')).toThrow(
                'Invalid hex string'
            );
        });

        test('fromBigIntHex with hex string without 0x prefix', () => {
            expect(() => fromBigIntHex('75bcd15')).toThrow(
                'Invalid hex string'
            );
        });
    ...
    describe('Hash Tests', () => {
        ...
        test('keccak function with invalid bits', () => {
            const data = Buffer.from([1, 2, 3, 4, 5]);
            expect(() => keccak(data, 128)).toThrow(
                'Invald algorithm: keccak128'
            );
        });
