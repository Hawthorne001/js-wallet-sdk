...
export const signData = async (
    ...
    await Crypto.ready();
    const addr = hex2Uint8Array(
        (await addressFromHexOrBech32(address)).toBytes()
    );
    ...
    const privKey = Crypto.Ed25519PrivateKey.fromExtendedBytes(
        prefix === 'addr_vkh' ? p.slice(0, 64) : p.slice(64)
    );
...
const extractKeyHash = async (address: string) => {
    ...
    try {
        ...
        return base.toBech32(
            'addr_vkh',
            base.fromHex(addr.getPaymentCredential().hash)
        );
    } catch (e) {}
    try {
