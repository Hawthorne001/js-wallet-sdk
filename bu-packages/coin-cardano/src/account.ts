...
export async function addressFromPubKey(publicKey: string) {
    ...
    const paymentHash = base.blake2b(pubKey.slice(0, 32), {
        dkLen: 28,
        key: undefined,
    });
    const stakeHash = base.blake2b(pubKey.slice(32), {
        dkLen: 28,
        key: undefined,
    });
    ...
    const paymentCredential: Cardano.Credential = {
        ...
        hash: Crypto.Hash28ByteBase16(base.toHex(paymentHash)),
    };
    const stakeCredential: Cardano.Credential = {
        type: Cardano.CredentialType.KeyHash,
        hash: Crypto.Hash28ByteBase16(base.toHex(stakeHash)),
    };
...
export async function getDerivedPrivateKey(mnemonic: string, path: string) {
    ...
    const rootKey = Crypto.Bip32PrivateKey.fromBip39Entropy(
        base.fromHex(entropy),
        ''
    );
    ...
    const keyToRawKeyHex = (key: Crypto.Bip32PrivateKey) =>
        key.toRawKey().hex();
