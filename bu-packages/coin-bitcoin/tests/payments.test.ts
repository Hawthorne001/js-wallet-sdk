...
describe('Bitcoin Payments Comprehensive Tests', () => {
    ...
    const validPubkey2 = Buffer.from(
        '03c6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee5',
        'hex'
    );
    const validSignature = Buffer.from(
        '304402207fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a002202fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a001',
        'hex'
    );
    const validHash = Buffer.from(
        '751e76c64647561e0b75f78e7e8f83c9d0a4b5c6',
        'hex'
    );
    const validHash32 = Buffer.from(
        '751e76c64647561e0b75f78e7e8f83c9d0a4b5c6751e76c64647561e0b75f78e',
        'hex'
    );
    ...
    describe('bip341 module comprehensive tests', () => {
        test('should handle rootHashFromPath with control block too small', () => {
            ...
            expect(() =>
                bip341.rootHashFromPath(controlBlock, leafHash)
        ...
        test('should handle tapleafHash with different versions', () => {
            ...
            const leafHash1 = bip341.tapleafHash({
                output: script,
                version: 0xc0,
            });
            const leafHash2 = bip341.tapleafHash({
                output: script,
                version: 0xc1,
            });
    ...
    describe('p2data (embed) payment tests', () => {
        ...
        test('should handle output validation with non-OP_RETURN start', () => {
            ...
            expect(() => payments.embed({ output: invalidOutput })).toThrow(
                'Output is invalid'
            );
        });
        ...
        test('should handle output validation with non-buffer chunks', () => {
            ...
            expect(() => payments.embed({ output: outputWithOpCode })).toThrow(
                'Output is invalid'
            );
        });
        ...
        test('should handle stacksEqual validation with different data', () => {
            ...
            expect(() => payments.embed({ data, output: wrongOutput })).toThrow(
                'Data mismatch'
            );
        });
        ...
        test('should test stacksEqual with different lengths', () => {
            ...
            expect(() =>
                payments.embed({ data: data2, output: output1 })
    ...
    describe('p2ms (multisig) payment tests', () => {
        ...
        test('should throw when m <= 0', () => {
            const invalidOutput = Buffer.from(
                '005121210279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f8179852ae',
                'hex'
            ); // 0-of-2
        ...
        test('should throw when m > n', () => {
            const invalidOutput = Buffer.from(
                '535121210279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f8179852ae',
                'hex'
            ); // 3-of-2
            expect(() => payments.p2ms({ output: invalidOutput })).toThrow(
                'Output is invalid'
        ...
        test('should throw when pubkey count mismatch with m and n', () => {
            expect(() =>
                payments.p2ms({ m: 1, n: 3, pubkeys: [pubkey1, pubkey2] })
        ...
        test('should check that output validation throws on invalid m > n pattern', () => {
            ...
            const validOutput = Buffer.from(
                '515121210279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f8179852ae',
                'hex'
            ); // 1-of-2
            expect(() => payments.p2ms({ m: 3, output: validOutput })).toThrow(
                'Output is invalid'
            ); // m > n check
        });
        ...
        test('should handle too many signatures when validation is disabled', () => {
            const payment = payments.p2ms(
                { m: 1, signatures: [validSignature, validSignature] },
                { validate: false }
            );
            expect(payment.signatures).toEqual([
                validSignature,
                validSignature,
            ]);
        });
        ...
        test('should throw when input is invalid (not starting with OP_0)', () => {
            const invalidInput = Buffer.from(
                '51' + validSignature.toString('hex'),
                'hex'
            ); // OP_1 + signature
            expect(() => payments.p2ms({ input: invalidInput, m: 1 })).toThrow(
                'Input is invalid'
            );
        });
        ...
        test('should handle valid input with proper structure', () => {
            const validInput = Buffer.from(
                '00' + validSignature.toString('hex'),
                'hex'
            ); // OP_0 + 1 signature
            const payment = payments.p2ms(
                { input: validInput, m: 1 },
                { validate: false }
            );
        ...
        test('should decode existing multisig output', () => {
            const output = Buffer.from(
                '515121' + validPubkey.toString('hex') + '52ae',
                'hex'
            ); // 1-of-1
        ...
        test('should handle input validation for invalid start', () => {
            ...
            expect(() => payments.p2ms({ input: badInput })).toThrow(
                'Input is invalid'
            );
        });
        ...
        test('should throw when signature mismatch between input and signatures (line ~157)', () => {
            ...
            const sig2 = Buffer.from(
                '304402207fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a002202fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a002',
                'hex'
            );
        ...
        test('should handle m and n mismatch validation', () => {
            ...
            const output = Buffer.concat([
                ...
                Buffer.from([validPubkey.length]),
                validPubkey, // First pubkey
                Buffer.from([validPubkey2.length]),
                validPubkey2, // Second pubkey
    ...
    describe('p2pk payment tests', () => {
        ...
        test('should handle output validation errors', () => {
            const badOutput = Buffer.from(
                '21' + validPubkey.toString('hex') + '00',
                'hex'
            ); // pubkey + OP_0
            expect(() => payments.p2pk({ output: badOutput })).toThrow(
                'Output is invalid'
            );
        });
        ...
        test('should throw when output is invalid (missing OP_CHECKSIG)', () => {
            ...
            expect(() => payments.p2pk({ output: invalidOutput })).toThrow(
                'Output is invalid'
            );
        });
        ...
        test('should handle invalid pubkey in output', () => {
            ...
            expect(() => payments.p2pk({ output: badPubkeyOutput })).toThrow(
        ...
        test('should throw when output pubkey is invalid', () => {
            ...
            const invalidOutput = Buffer.from(
                '01' + invalidPubkey.toString('hex') + 'ac',
                'hex'
            );
            expect(() => payments.p2pk({ output: invalidOutput })).toThrow(
                'Output pubkey is invalid'
        ...
        test('should handle pubkey mismatch', () => {
            const output = Buffer.from(
                '21' + validPubkey.toString('hex') + 'ac',
                'hex'
            );
            expect(() =>
                payments.p2pk({ pubkey: validPubkey2, output })
        ...
        test('should handle invalid signature in input', () => {
            ...
            expect(() => payments.p2pk({ input: invalidSigInput })).toThrow(
        ...
        test('should throw when input has invalid signature', () => {
            ...
            const input = Buffer.from(
                '01' + invalidSignature.toString('hex'),
                'hex'
            );
            expect(() => payments.p2pk({ input })).toThrow(
                'Input has invalid signature'
            );
        });
    ...
    describe('p2pkh payment tests', () => {
        ...
        test('should handle output format validation', () => {
            const badOutput = Buffer.from(
                '76a914' + validHash.toString('hex') + '87ac',
                'hex'
            ); // Missing OP_EQUALVERIFY
            expect(() => payments.p2pkh({ output: badOutput })).toThrow(
                'Output is invalid'
            );
        });
        ...
        test('should throw when invalid output format', () => {
            ...
            expect(() => payments.p2pkh({ output: invalidOutput })).toThrow(
                'Output is invalid'
            );
        });
        ...
        test('should handle hash mismatch', () => {
            ...
            expect(() =>
                payments.p2pkh({ hash: hash1, pubkey: validPubkey })
        ...
        test('should handle invalid signature in input', () => {
            const badSigInput = Buffer.from(
                '01ff21' + validPubkey.toString('hex'),
                'hex'
            );
            expect(() => payments.p2pkh({ input: badSigInput })).toThrow(
                'Input has invalid signature'
            );
        });
        ...
        test('should throw when input has invalid signature', () => {
            ...
            const input = Buffer.from(
                '01' +
                    invalidSig.toString('hex') +
                    '21' +
                    validPubkey.toString('hex'),
                'hex'
            );
            expect(() => payments.p2pkh({ input })).toThrow(
                'Input has invalid signature'
            );
        });
        ...
        test('should validate input format with valid signature but invalid pubkey', () => {
            const input = Buffer.from(
                '48' +
                    validSignature.toString('hex') +
                    '01' +
                    Buffer.from('00', 'hex').toString('hex'),
                'hex'
            );
            expect(() => payments.p2pkh({ input })).toThrow(
                'Input has invalid signature'
            ); // Will fail on signature first
        });
    ...
    describe('p2wpkh payment tests', () => {
        ...
        test('should handle invalid witness length', () => {
            ...
            expect(() => payments.p2wpkh({ witness: shortWitness })).toThrow(
                'Witness is invalid'
            );
        });
        ...
        test('should throw when witness is invalid', () => {
            ...
            expect(() => payments.p2wpkh({ witness })).toThrow(
                'Witness is invalid'
            );
        });
        ...
        test('should handle invalid signature in witness', () => {
            ...
            expect(() => payments.p2wpkh({ witness: badSigWitness })).toThrow(
        ...
        test('should validate output buffer length', () => {
            const invalidOutput = Buffer.from(
                '0014' + validHash.toString('hex') + 'aa',
                'hex'
            ); // Wrong length
        ...
        test('should validate pubkey format', () => {
            const invalidPubkey = Buffer.from(
                '02' + validPubkey.slice(1, -1).toString('hex'),
                'hex'
    ...
    describe('p2sh payment tests', () => {
        ...
        test('should handle output format validation', () => {
            const badOutput = Buffer.from(
                'a914' + validHash.toString('hex') + '88',
                'hex'
            ); // Wrong ending
            expect(() => payments.p2sh({ output: badOutput })).toThrow(
                'Output is invalid'
            );
        });
        ...
        test('should throw when invalid output format', () => {
            ...
            expect(() => payments.p2sh({ output: invalidOutput })).toThrow(
                'Output is invalid'
            );
        });
        ...
        test('should handle empty redeem output', () => {
            ...
            expect(() => payments.p2sh({ redeem: emptyRedeem })).toThrow(
        ...
        test('should throw when redeem output is too short', () => {
            ...
            expect(() => payments.p2sh({ redeem })).toThrow(
                'Redeem.output too short'
            );
        });
        ...
        test('should handle empty input validation', () => {
            ...
            expect(() => payments.p2sh({ redeem: emptyInputRedeem })).toThrow(
                'Empty input'
            );
        });
        ...
        test('should handle both input and witness provided', () => {
            ...
            expect(() => payments.p2sh({ redeem: bothInputRedeem })).toThrow(
        ...
        test('should handle redeem output decompile that produces invalid output', () => {
            ...
            expect(() => payments.p2sh({ input: invalidInput })).toThrow(
                'Input is invalid'
            );
        });
        ...
        test('should handle input too short validation', () => {
            ...
            expect(() => payments.p2sh({ input: emptyInput })).toThrow(
                'Input too short'
            );
        });
    ...
    describe('p2wsh payment tests', () => {
        ...
        test('should handle ambiguous witness source', () => {
            ...
            expect(() => payments.p2wsh({ redeem: ambiguousRedeem })).toThrow(
        ...
        test('should handle invalid redeem output', () => {
            ...
            expect(() =>
                payments.p2wsh({ redeem: invalidRedeemOutput })
            ).toThrow('Redeem.output is invalid');
        });
    ...
    describe('Validation and option handling', () => {
        test('should bypass validation when validate is false', () => {
            ...
            expect(() =>
                payments.p2ms(
                    { m: 1, pubkeys: [validPubkey] },
                    { validate: false }
                )
            ).not.toThrow();
            expect(() =>
                payments.p2pk({ pubkey: validPubkey }, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2pkh({ pubkey: validPubkey }, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2wpkh({ pubkey: validPubkey }, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2sh(
                    { redeem: { output: Buffer.from('51', 'hex') } },
                    { validate: false }
                )
            ).not.toThrow();
            expect(() =>
            ...
            expect(() =>
                payments.p2tr({ internalPubkey }, { validate: false })
            ).not.toThrow();
        });
        ...
        test('should bypass validation for some payment types', () => {
            ...
            expect(() =>
                payments.p2ms(multisigData, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2pk(validData, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2pkh(validData, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2wpkh(validData, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2sh(redeemData, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2wsh(redeemData, { validate: false })
            ).not.toThrow();
            expect(() =>
                payments.p2tr(p2trData, { validate: false })
            ).not.toThrow();
        });
        ...
        test('should test embed with required data', () => {
            ...
            expect(() =>
                payments.embed({ data }, { validate: false })
            ).not.toThrow();
        });
    });
