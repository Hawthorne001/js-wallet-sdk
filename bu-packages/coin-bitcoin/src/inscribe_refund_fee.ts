...
export class InscriptionRefundFeeTool {
    ...
    static newInscriptionRefundFeeTool(
        network: bitcoin.Network,
        request: InscriptionRefundFeeRequest
    ) {
        ...
        const insufficient = tool.buildrRefundFeeTx(
            network,
            request.inputs.slice(0, amountOfInputs),
    ...
    buildrRefundFeeTx(
        ...
        const vsize = countAdjustedVsize(
            txForEstimate,
            prevOutputList.map((a) => a.address),
            network
        );
...
function signTx(
    ...
    refundFeeTxPrevOutputList: PrevOutput[],
    network: bitcoin.Network,
    merkleRoot = new Uint8Array()
) {
    tx.ins.forEach((input, i) => {
        const addressType = getAddressType(
            refundFeeTxPrevOutputList[i].address,
            network
        );
        const privateKey = base.fromHex(
            privateKeyFromWIF(refundFeeTxPrevOutputList[i].privateKey, network)
        );
        ...
        if (addressType === 'segwit_taproot') {
            const prevOutScripts = refundFeeTxPrevOutputList.map((o) =>
...
export function createInscriptionTxCtxData(
    network: bitcoin.Network,
    inscriptionData: InscriptionRefundFeeData,
...
export function inscribeRefundFee(
    network: bitcoin.Network,
    request: InscriptionRefundFeeRequest
) {
    const tool = InscriptionRefundFeeTool.newInscriptionRefundFeeTool(
        network,
        request
    );
