...
export class SrcInscriptionTool {
    ...
    static newSrcInscriptionTool(
        network: bitcoin.Network,
        request: SrcInscriptionRequest
    ) {
    ...
    buildCommitTx(
        ...
        let buf = base.fromHex(
            xcp_rc4(commitTxPrevOutputList[0].txId, data.toString('hex'))
        );
        ...
        tx.addOutput(
            bitcoin.address.toOutputScript(inscriptionData.revealAddr, network),
            revealOutValue
        );
        while (buf.length) {
