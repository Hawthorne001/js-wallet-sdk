    ...
    static newInscriptionTool(
        ...
        request.inscriptionDataList.forEach((inscriptionData) => {
            tool.inscriptionTxCtxDataList.push(
                createInscriptionTxCtxData(network, inscriptionData, privateKey)
            );
        });
...
export function inscribeForMPCUnsigned(
    request: InscriptionRequest,
    network: bitcoin.Network,
    unsignedCommitTxHash?: Buffer,
    signedCommitTxHash?: Buffer
) {
    const privateKey = request.commitTxPrevOutputList[0].privateKey;
    ...
    request.commitTxPrevOutputList.forEach((uxto) => {
        commitTx.addInput(
            base.reverseBuffer(base.fromHex(uxto.txId)),
            uxto.vOut,
            defaultSequenceNum
        );
    ...
    scriptCtxList.forEach((ctx) => {
        commitTx.addOutput(
            ctx.revealTxPrevOutput.pkScript,
            ctx.revealTxPrevOutput.value
        );
    ...
    const vsize = countAdjustedVsize(
        estimateTx,
        request.commitTxPrevOutputList.map((a) => a.address),
        network
    );
    ...
    if (changeValue >= (request.minChangeValue || defaultMinChangeValue)) {
        ...
        const feeWithoutChange = Math.floor(
            vsizeWithoutChange * request.commitFeeRate
        );
    ...
    const sigHashList = calculateSigHash(
        commitTx,
        request.commitTxPrevOutputList,
        network
    );
    ...
    revealTxList.forEach((revealTx, i) => {
        ...
        const sigHash = revealTx.hashForWitnessV1(
            ...
            values,
            bitcoin.Transaction.SIGHASH_DEFAULT,
            scriptCtxList[i].hash
        );
        const signature = Buffer.from(
            schnorr.sign(
                sigHash,
                scriptCtxList[i].privateKey,
                base.randomBytes(32)
            )
        );
...
export function inscribeForMPCSigned(
    request: InscriptionRequest,
    network: bitcoin.Network
) {
    ...
    const res = inscribeForMPCUnsigned(
        request,
        network,
        unsignedCommitTxHash,
        signedCommitTxHash
    );
    return {
        signHashList: null,
...
function calculateSigHash(
    tx: bitcoin.Transaction,
    prevOutFetcher: PrevOutput[],
    network: bitcoin.Network
) {
    const sigHashList: string[] = [];
    tx.ins.forEach((input, i) => {
        const publicKey = base.fromHex(prevOutFetcher[i].publicKey!);
        const pkScript = bitcoin.address.toOutputScript(
            prevOutFetcher[i].address,
            network
        );
        ...
        let sigHash;
        if (isP2TR(pkScript)) {
            const prevOutScripts = prevOutFetcher.map((o) =>
                bitcoin.address.toOutputScript(o.address, network)
            );
            const values = prevOutFetcher.map((o) => o.amount);
            sigHash = tx.hashForWitnessV1(
                i,
                prevOutScripts,
                values,
                bitcoin.Transaction.SIGHASH_DEFAULT
            ...
            sigHash = tx.hashForWitness(
                i,
                prevOutScript,
                prevOutFetcher[i].amount,
                bitcoin.Transaction.SIGHASH_ALL
            );
