...
export class TxBuild {
    ...
    build(hashArray?: string[]): string {
        ...
        for (let i = 0; i < eckeys.length; i++) {
            ...
            if (this.bitcoinCash) {
                const script = bitcoin.payments.p2pkh({ pubkey: ecPub })
                    .output as Buffer;
                hashType =
    ...
    virtualSize() {
        return countAdjustedVsize(
            this.tx,
            this.inputs.map((a) => a.address),
...
export function signBtc(
    ...
    network = network || bitcoin.networks.bitcoin;
    if (utxoTx.memo) {
        let buf = base.isHexString(utxoTx.memo)
    ...
    if (!fakePrivateKey) {
        fakePrivateKey = private2Wif(
            base.fromHex(
                '853fd8960ff34838208d662ecd3b9f8cf413e13e0f74f95e554f8089f5058db0'
            ),
            network
        );
    }
    ...
    let changeAmount =
        inputAmount - outputAmount - Math.ceil(virtualSize * feePerB);
    ...
    for (let i = 0; i < outputs.length; i++) {
        ...
        if (
            utxoTx.memo &&
            utxoTx.memoPos &&
            txBuild.outputs.length == utxoTx.memoPos
        ) {
            txBuild.addOutput(
                '',
                0,
...
export function calculateTxSize(
    ...
    if (
        memo &&
        (pos == undefined || pos < 0 || pos > preTxBuild.outputs.length)
    ) {
        preTxBuild.addOutput(
            '',
            0,
...
export function calculateBchTxSize(
    inputs: [],
    outputs: [],
    changeAddress: string,
    privateKey: string,
    ...
    for (let i = 0; i < transaction.ins.length; i++) {
        ...
        if (bitcoinCash) {
            ...
            if (addressType === 'legacy') {
                ...
                input.witness[0] = bitcoin.script.signature.encode(
                    signature,
                    hashType
                );
            }
...
export function ValidSignedTransaction(
    ...
    for (let i = 0; i < transaction.ins.length; i++) {
        ...
        if (addressType === 'legacy') {
            ...
            if (
                !schnorr.verify(
                    base.toHex(signature),
                    base.toHex(hash),
                    base.toHex(tweakedPubKey)
                )
            ) {
                throw Error('signature error');
            }
...
export function estimateBchFee(utxoTx: utxoTx, network?: bitcoin.Network) {
    ...
    let { virtualSize } = calculateBchTxSize(
        inputs,
        outputs,
        utxoTx.address,
        fakePrivateKey,
