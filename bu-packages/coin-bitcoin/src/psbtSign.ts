...
export function classicToPsbt(
    ...
    tx.inputs.forEach((input: utxoInput) => {
        ...
        if (input.bip32Derivation) {
            if (addressType === 'segwit_taproot') {
                inputData.tapBip32Derivation = input.bip32Derivation!.map(
                    (derivation: any) => {
                        ...
                        return {
                            masterFingerprint: base.fromHex(
                                derivation.masterFingerprint
                            ),
                            pubkey: pubBuf,
                            path: derivation.path,
                            leafHashes: derivation.leafHashes.map(
                                (leaf: any) => {
                ...
                inputData.bip32Derivation = input.bip32Derivation!.map(
                    (derivation: any) => {
                        return {
                            masterFingerprint: base.fromHex(
                                derivation.masterFingerprint
        ...
        if (addressType === 'legacy') {
            ...
            if (input.publicKey) {
                inputData.tapInternalKey = toXOnly(
                    base.fromHex(input.publicKey)
                );
            }
    ...
    tx.outputs.forEach((output: utxoOutput) => {
        if (output.omniScript) {
            psbt.addOutput({
                script: base.fromHex(output.omniScript),
                value: 0,
            });
        } else {
            let outputData: PsbtOutputExtended = {
                address: output.address,
                value: output.amount,
            };
            if (output.bip32Derivation) {
                outputData.bip32Derivation = output.bip32Derivation!.map(
                    (derivation: any) => {
                        return {
                            masterFingerprint: base.fromHex(
                                derivation.masterFingerprint
    ...
    try {
        const psbt = Psbt.fromHex(psbtBase64, {
            ...
            maximumFeeRate: maximumFeeRate
                ? maximumFeeRate
                : defaultMaximumFeeRate,
        });
        return psbt.txInputs
            ? psbt.txInputs
                  .filter((a) => !a.hash.equals(Buffer.alloc(32)))
                  .map((a) => {
                      return {
                          txId: base.toHex(base.reverseBuffer(a.hash)),
                          vOut: a.index,
                      };
                  })
            : [];
...
export function signPsbtWithKeyPathAndScriptPathBatch(
    ...
    for (i = 0; i < psbtHexs.length; i++) {
        ...
        const signedPsbt = signPsbtWithKeyPathAndScriptPath(
            psbtHexs[i],
            privateKey,
            network,
            {
...
export function signPsbtWithKeyPathAndScriptPath(
    ...
    signPsbtWithKeyPathAndScriptPathImpl(
        psbt,
        privateKey,
        network,
        opts.autoFinalized,
...
export function signPsbtWithKeyPathAndScriptPathImpl(
    ...
    const signer = {
        ...
        signSchnorr(hash: Buffer): Buffer {
            let tweakedPrivKey = taproot.taprootTweakPrivKey(
                base.fromHex(privKeyHex)
            );
            if (this.toSignInputsMap?.has(this.psbtIndex)) {
                if (
                    this.toSignInputsMap.get(this.psbtIndex)?.disableTweakSigner
                ) {
                    // tweakedPrivKey = base.fromHex(privKeyHex);
                    return Buffer.from(
                        schnorr.sign(hash, privKeyHex, base.randomBytes(32))
                    );
                }
            }
            if (!this.needTweak) {
            ...
            if (this.needTweak && this.tweakHash.length > 0) {
                tweakedPrivKey = taproot.taprootTweakPrivKey(
                    base.fromHex(privKeyHex),
                    this.tweakHash
                );
            }
    ...
    const getSigner = () => {
        return {
            ...
            getPrivateKey() {
                ...
                if (this.needTweak) {
                    privKey = Buffer.from(
                        taproot.taprootTweakPrivKey(privKey, this.tweakHash)
                    );
                }
                return privKey;
            },
    ...
    for (let i = 0; i < psbt.inputCount; i++) {
        ...
        if (signInputMap?.get(i)?.useTweakSigner !== undefined) {
            ...
            if (forUseTweakSigner.needTweak) {
                forUseTweakSigner.publicKey = Buffer.from(
                    taproot.taprootTweakPubkey(
                        toXOnly(wif2Public(privateKey, network)),
                        forUseTweakSigner.tweakHash
                    )[0]
                );
            }
            ...
            if (isTaprootInput(input)) {
                if (!input.tapInternalKey && !input.tapLeafScript) {
                    // don't need internal key if have leaf script
                    input.tapInternalKey = toXOnly(
                        wif2Public(privateKey, network)
                    );
                }

                if (signInput.tapLeafHashToSign !== undefined) {
                    tapLeafHashToSign = Buffer.from(
                        signInput.tapLeafHashToSign,
                        'hex'
                    );
                }
            }
...
export function psbtSignImpl(
    ...
    const signer = {
        ...
        signSchnorr(hash: Buffer): Buffer {
            const tweakedPrivKey = taproot.taprootTweakPrivKey(
                base.fromHex(privKeyHex)
            );
            return Buffer.from(
                schnorr.sign(hash, tweakedPrivKey, base.randomBytes(32))
...
export function extractPsbtTransaction(
    txHex: string,
    network?: Network,
    maximumFeeRate?: number
) {
...
export function generateUnsignedListingPsbt(
    ...
    let placeholderAddress =
        'bc1pcyj5mt2q4t4py8jnur8vpxvxxchke4pzy7tdr9yvj3u3kdfgrj6sw3rzmr';
    if (network === networks.testnet) {
        placeholderAddress =
            'tb1pcyj5mt2q4t4py8jnur8vpxvxxchke4pzy7tdr9yvj3u3kdfgrj6see4dpv';
    }
    tx.inputs.push(
        {
            txId: '0'.repeat(64),
...
export function generateSignedListingPsbt(
    listingData: ListingData,
    privateKey: string,
    network?: Network
) {
    ...
    return psbtSign(
        generateUnsignedListingPsbt(listingData, network, publicKey),
        privateKey,
        network
    );
}
...
export function generateUnsignedBuyingPsbt(
    ...
    tx.outputs.push({
        address: buyingData.paymentAndChangeAddress,
        amount: buyingData.dummyUtxos.reduce(
            (sum, dummyUtxo) => sum + dummyUtxo.coinAmount,
            0
        ),
    } as never);
    ...
    buyingData.sellerPsbts.forEach((sellerPsbt) => {
        ...
        const nftInput = (psbt.data.globalMap.unsignedTx as any).tx.ins[
            SELLER_INDEX
        ];
        nftOutputs.push(
            (psbt.data.globalMap.unsignedTx as any).tx.outs[SELLER_INDEX]
        );
        let nftUtxo = psbt.data.inputs[SELLER_INDEX].witnessUtxo;
        if (!nftUtxo) {
            nftUtxo = Transaction.fromBuffer(
                psbt.data.inputs[SELLER_INDEX].nonWitnessUtxo!
            ).outs[nftInput.index];
        }
    ...
    const changeAmount = parseInt(
        signBtc(tx, '', network, undefined, false, true)
    );
...
export function mergeSignedBuyingPsbt(
    signedBuyingPsbt: string,
    signedListingPsbts: string[]
) {
    ...
    signedListingPsbts.forEach((signedListingPsbt, i) => {
        ...
        (buyerSignedPsbt.data.globalMap.unsignedTx as any).tx.ins[
            nftIndex + i
        ] = (sellerSignedPsbt.data.globalMap.unsignedTx as any).tx.ins[
            SELLER_INDEX
        ];
...
export function generateSignedBuyingTx(
    buyingData: BuyingData,
    privateKey: string,
    network?: Network
) {
    const publicKey = base.toHex(wif2Public(privateKey, network));
    const signedBuyingPsbt = psbtSign(
        generateUnsignedBuyingPsbt(buyingData, network, publicKey),
        privateKey,
        network
    );
    return extractPsbtTransaction(
        mergeSignedBuyingPsbt(signedBuyingPsbt, buyingData.sellerPsbts).toHex(),
        network
    );
}

// note brc20-mpc not support taproot address
export function generateMPCUnsignedListingPSBT(
    psbtBase64: string,
    pubKeyHex: string,
    network?: Network
) {
...
export function generateMPCUnsignedPSBT(
    psbtStr: string,
    pubKeyHex: string,
    network?: Network
) {
    ...
    for (let i = 0; i < psbt.inputCount; i++) {
        try {
            const { hash } = psbt.getHashAndSighashType(
                i,
                publicKey,
                allowedSighashTypes
            );
...
export function generateMPCSignedPSBT(
    psbtStr: string,
    pubKeyHex: string,
    signatureList: string[],
    network?: Network
