...
export const dogeCoin: bitcoin.Network = {
    ...
    bech32: '', // Dogecoin does not support segwit addresses
    bip32: {
        public: 49990397,
        private: 49988504,
    },
...
export class DogeWallet extends BtcWallet {
        ...
        if (type === 1) {
            ...
            try {
                return Promise.resolve(
                    bitcoin.dogInscribe(dogeCoin, param.data)
                );
            } catch (e) {
    ...
    signMessage0(param: SignTxParams): Promise<string> {
        try {
            ...
            let signature = bitcoin.message.sign(
                param.privateKey,
                typedMessage.message,
                this.network(),
                dogeCoin.messagePrefix
            );
