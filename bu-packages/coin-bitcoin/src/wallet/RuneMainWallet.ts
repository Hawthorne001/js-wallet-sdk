...
export class RuneMainWallet extends BtcWallet {
    convert2RuneTx(paramData: any): utxoTx {
        ...
        if (
            clonedParamData.runeData.useDefaultOutput == true &&
        ...
        // Prevent setting usedefaultOutput but lacking defaultOutput
        if (
            clonedParamData.runeData.mint == true &&
            ...
            if (dataArray != null && dataArray instanceof Array) {
                for (const data of dataArray) {
                    ...
                    if (runeId.split(':').length <= 1) {
                        throw new Error(
                            JSON.stringify({
                                errCode: ErrCodeRuneIdNotStandard,
                                date: { runeId: runeId },
                            })
                        );
                    }
    ...
    async signTransaction(param: SignTxParams): Promise<any> {
        try {
            ...
            if (param.data.runeData.etching) {
                return runesMainInscribe(
                    network,
                    param.data as RunesMainInscriptionRequest
                );
            } else if (
                param.data.runeData.mintNum == undefined ||
                ...
                const opReturnOutput = this.getRuneMainOpReturnOutput(
                    network,
                    runeTx.runeData!
                );
                ...
                const opMintReturnOutput = this.getRuneMainOpReturnOutput(
                    network,
                    baseMintTx.runeData!
                );
                ...
                const baseMintfee =
                    bitcoin.estimateBtcFee(baseMintTx, this.network()) + 546;
                ...
                for (let i = 0; i < mintData.mintNum - 1; i++) {
                    ...
                    batchMintStatNum += 1;
                    let curSubTx = this.getMinRuneTx(
                        param.data,
                        mintData,
                        curInput,
                        curOutput
                ...
                let baseMintTx = this.getMockMinRuneTx(param.data, mintData);
                const opMintReturnOutput = buildRuneMainMintOp(
                    mintData.id,
                    false,
                    0,
                    true
                ...
                baseMintTx.outputs.push(opMintReturnOutput as never);
                const baseMintfee = bitcoin.estimateBtcFee(
                    baseMintTx,
                    this.network()
                );
                let curAmount = (mintData.mintNum - 1) * baseMintfee + 546;

                const runeTx = this.convert2RuneTxSerialMint(
                    param.data,
                    curAmount
                );
    ...
    async buildPsbt(param: SignTxParams): Promise<any> {
        ...
        try {
            ...
            runeTx.outputs.push(opReturnOutput as never);
            let fakeAddr =
                'KwdkfXMV2wxDVDMPPuFZsio3NeCskAUd4N2U4PriTgpj2MqAGmmc';
