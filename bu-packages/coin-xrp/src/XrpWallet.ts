...
export class XrpWallet extends BaseWallet {
    ...
    checkPrivateKey(privateKey: string) {
        if (isHexStr(privateKey)) {
            ...
            if (keyBytes.length == 33) {
                if (
                    !privateKey.toUpperCase().startsWith(PREFIX_ED25519) &&
                    !privateKey.toUpperCase().startsWith(PREFIX_SECP256K1)
                ) {
                    return false;
                }
            ...
            if (
                privateKey.toLowerCase() == ZERO_SEED_ED25519.toLowerCase() ||
    ...
    validAddress(param: ValidAddressParams): Promise<any> {
        // Accept classic addresses (r prefix) and mainnet X-addresses (X prefix)
        // Reject testnet X-addresses (T prefix)
        // Note: 'r' addresses can be on both mainnet and testnet, but we accept them
        const hasValidPrefix =
            param.address.startsWith('r') || param.address.startsWith('X');
        const isValid = !isInValidAddress(param.address) && hasValidPrefix;
    ...
    async signMessage(param: SignTxParams): Promise<string> {
        ...
        return Promise.resolve(
            sign(transferMsgPayload(msg), w.privateKey).toString()
        );
    }
    ...
    verifyMessage(param: VerifyMessageParams): Promise<boolean> {
        ...
        return Promise.resolve(
            verify(
                transferMsgPayload(msg.message),
                param.signature,
                msg.publicKey
            )
        );
