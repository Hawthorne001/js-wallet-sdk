...
import { isValidClassicAddress, isValidXAddress, TrustSet, Wallet } from 'xrpl';
import {
    Amount,
    ...
    Path,
    Signer,
} from 'xrpl/src/models/common';
import {
    Payment,
    PaymentFlagsInterface,
} from 'xrpl/src/models/transactions/payment';
...
export function isInValidAddress(addr: string) {
    if (INVALID_ADDRESS.includes(addr)) {
        return true;
    }

    return !isValidClassicAddress(addr) && !isValidXAddress(addr);
}
...
function convertTxParamInner(xrpParam: XrpParam, account: string) {
    const baseParm = xrpParam.base;
    const sequence =
        typeof baseParm.sequence == 'string'
    ...
    switch (xrpParam.type) {
        case 'transfer': {
            ...
            if (isInValidAddress(txP.destination)) {
                throw new Error(
                    `Invalid destination address: ${txP.destination}`
                );
            }
...
export function createWallet(seedOrPrivate: string, addressType?: string) {
    if (isHexStr(seedOrPrivate)) {
        ...
        if (seedOrPrivate.startsWith('00') && seedOrPrivate.length == 66) {
            let pubKey = signUtil.secp256k1.publicKeyCreate(
                base.fromHex(seedOrPrivate.slice(2)),
                true
            );
            pubKeyHex = base.toHex(pubKey);
        } else if (
            seedOrPrivate.startsWith('ED') &&
            seedOrPrivate.length == 66
        ) {
            let pubKey = signUtil.ed25519.publicKeyCreate(
                base.fromHex(seedOrPrivate.slice(2))
            );
