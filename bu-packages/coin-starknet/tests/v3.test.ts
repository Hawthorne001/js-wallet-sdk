...
const privateKey =
    '0x5283459ec65271070e2c7f67917bc66774eb08dad406d54d766270267162ecf';
...
describe('account', () => {
    ...
    test('getDerivedPrivateKey', async () => {
        ...
        // 测试相同助记词和路径应该产生相同的私钥
        const privateKey2 = await wallet.getDerivedPrivateKey({
            mnemonic,
            hdPath,
        });
        ...
        const privateKey3 = await wallet.getDerivedPrivateKey({
            mnemonic,
            hdPath: differentPath,
        });
    ...
    test('getDerivedPrivateKeyOld (legacy method)', async () => {
        ...
        if (walletWithPrivate.getDerivedPrivateKeyOld) {
            const privateKey = await walletWithPrivate.getDerivedPrivateKeyOld({
                mnemonic,
                hdPath,
            });
            expect(privateKey).toMatch(/^0x[0-9a-fA-F]+$/);
    ...
    test('getDerivedPrivateKey branch coverage - childKey.privateKey null', async () => {
        ...
        } as any).mockReturnValueOnce({ privateKey: null } as any);
    ...
    test('getDerivedPrivateKeyOld branch coverage - ethKey.privateKey null', async () => {
        ...
        try {
            ...
            if (walletWithPrivate.getDerivedPrivateKeyOld) {
                await expect(
                    walletWithPrivate.getDerivedPrivateKeyOld({
                        mnemonic:
                            'abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about',
                        hdPath: "m/44'/9004'/0'/0/0",
                    })
    ...
    test('getDerivedPrivateKeyOld branch coverage - childKey.privateKey null', async () => {
        ...
        const mockDerivePath1 = jest.fn().mockReturnValue({
            privateKey: Buffer.from('test_private_key', 'hex'),
        });
        const mockDerivePath2 = jest.fn().mockReturnValue({ privateKey: null });

        spy.mockReturnValueOnce({
            derivePath: mockDerivePath1,
        } as any).mockReturnValueOnce({ derivePath: mockDerivePath2 } as any);
        ...
        try {
            ...
            if (walletWithPrivate.getDerivedPrivateKeyOld) {
                ...
                // 验证spy被调用了两次
                expect(spy).toHaveBeenCalledTimes(2);
                expect(mockDerivePath1).toHaveBeenCalledWith(
                    "m/44'/60'/0'/0/0"
                );
                expect(mockDerivePath2).toHaveBeenCalledWith(
                    "m/44'/9004'/0'/0/0"
                );
            }
        } finally {
    ...
    test('getNewAddress', async () => {
        const privateKey =
            '49c0722d56d6bac802bdf5c480a17c870d1d18bc4355d8344aa05390eb778280';
        const expectedAddress =
            '0x05b0563ae63a7ba9929129f5117bcbcbe552b7cc8cf81c806d343e2de7f3d555';
        expect(
    ...
    test('validAddress', async () => {
        ...
        // 测试另一个有效地址
        const validAddress2 =
            '0x027854ed5aa9f534c7aa0165c89b3915bc461f70e64ce9d64f4e2549037ebcf2';
        ...
        // 测试无效地址 - 没有0x前缀
        const noPrefix =
            '05b0563ae63a7ba9929129f5117bcbcbe552b7cc8cf81c806d343e2de7f3d555';
        const result4 = await wallet.validAddress({ address: noPrefix });
        expect(result4.isValid).toBe(false);

        // 测试无效地址 - 包含非hex字符
        const invalidHex =
            '0x05b0563ae63a7ba9929129f5117bcbcbe552b7cc8cf81c806d343e2de7f3d55g';
...
describe('message', () => {
    test('signCommonMsg', async () => {
        let sig = await wallet.signCommonMsg({
            privateKey:
                '0x0028e2e382f4e68b24154142dece87d03b6214a34834aede41e100848826c16b',
            message: { walletId: '12345678901234567890' },
        });
        ...
        sig = await wallet.signCommonMsg({
            privateKey:
                '0x0028e2e382f4e68b24154142dece87d03b6214a34834aede41e100848826c16b',
            message: { text: '12345678901234567890' },
        });
        expected =
            '';
        expect(sig).toEqual(expected);

        sig = await wallet.signCommonMsg({
            privateKey:
                '4b43bf0e3864122edff9f143006f0a0d61b16df3f676c8070dac1d0f42d78353',
            message: { walletId: 'AEEDD050-0DD9-43E6-9161-0B54E1D66E09' },
        });
        expected =
            '';
        expect(sig).toEqual(expected);
    });
    ...
    test('signMessage with TypedData', async () => {
        ...
        let sig = await wallet.signMessage({
            privateKey,
            data: { message: typedDataValidate },
        });
        const publicKey = signUtil.schnorr.stark.getPublicKey(privateKey);
        expect(
            verifyMessage(
                new Signature(sig.signature.r, sig.signature.s),
                sig.hash,
                publicKey
            )
        ).toEqual(true);
...
describe('tx', () => {
    ...
    const privateKey =
        '0x0248555dbad1b89eb17dca845a0fedc2ac02d2102737e3724241349fc6e4d3a1';
    ...
    test('signTransaction address validation', async () => {
        ...
        // Test valid Starknet addresses should pass
        const validAddresses = [
            '0x02dc53467fca68213dfef46091fd2985a40564e8841cb9795e48ca61e66d17cd',
            '0x053002233beeab93e45fa620ab40e82b26a0700f514915b3a15e091a4004c392',
            ...
            const result = await wallet.signTransaction({
                ...
                data: {
                    ...
                    transferData: {
                        contractAddress:
                            '0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7',
                        from: '0x053002233beeab93e45fa620ab40e82b26a0700f514915b3a15e091a4004c392',
                        to: address,
                        amount: '1000',
                    },
                },
                ...
                data: {
                    ...
                    transferData: {
                        contractAddress:
                            '0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7',
                        from: '0x053002233beeab93e45fa620ab40e82b26a0700f514915b3a15e091a4004c392',
                        to: '0x742d35Cc6634C0532925a3b844Bc454e4438f44e', // Ethereum address (too short)
                        amount: '1000',
                    },
                },
            })
        ...
        // Test invalid toAddress - Invalid hex format
        await expect(
            wallet.signTransaction({
                privateKey,
                data: {
        ...
        // Test invalid toAddress - Too short Starknet address
        await expect(
            wallet.signTransaction({
                privateKey,
                data: {
...
describe('utils', () => {
    ...
    test('calculated contract address should match the snapshot', () => {
        ...
        const factoryAddress =
            '0x249827618A01858A72B7D04339C47195A324D20D6037033DFE2829F98AFF4FC';
        const classHash =
            '0x55187E68C60664A947048E0C9E5322F9BF55F7D435ECDCF17ED75724E77368F';
    ...
    test('calculateContractAddressFromHash', async () => {
        const privateKey =
            '0x0603c85d20500520d4c653352ff6c524f358afeab7e41a511c73733e49c3075e';
        ...
        expect(contractAddress).toEqual(
            '0x06c3c93eeb1643740a80a338b9346c0c9a06177bfcc098a6d86e353532090ae4'
        );
    });
    ...
    test('hashMessage', async () => {
        ...
        expect(hash).toEqual(
            '0x113fceee9332ec6033c52ec5203629edd345120fd5f8bcacb68df848fe8f51e'
        );
    });
    ...
    test('modPrivateKey', () => {
        ...
        const pri2 =
            '0x800000000000010ffffffffffffffffb781126dcae7b2321e66a241adc64d3f';
        const res2 = modPrivateKey(pri2);
        expect(res2).toEqual('0x10');

        const pri3 =
            '0x800000000000010ffffffffffffffffb781126dcae7b2321e66a241adc64d2e';
        ...
        const pri4 =
            '800000000000010ffffffffffffffffb781126dcae7b2321e66a241adc64d2e';
