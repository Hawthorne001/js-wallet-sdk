...
import {
    ...
    createRawTransactionByABIV2,
    createRawTransactionV2,
    createSimulateRawTransactionByABIV2,
    generateBCSSimulateTransaction,
    generateBCSSimulateTransactionWithPublicKey,
    generateBCSTransaction,
    HexString,
    migrateToFaStorePayload,
    mintCoin,
...
import {
    Deserializer as DeserializerBcs,
    Serializer as SerializerBcs,
} from './transaction_builder/bcs';
...
export type AptosParam = {
    type:
        ...
        | 'claimNft_simulate'
        | 'simple_transaction'
        | 'simulate_simple_transaction'
        | 'fungible_asset_transfer'
        | 'simulate_fungible_asset_transfer'
        | 'tokenTransferV2'
        | 'signAsFeePayer'
        | 'signTx';
    base: AptosBasePram;
...
export type SignTxParam = {
    rawTxn: string;
    type: '1' | '2'; //1 represents simple tx, 2 represents multi agent tx, 3 represents script transaction
};

export type SignAsFeePayerParam = {
    rawTxn: string;
    type: '1' | '2'; //1 represents simple tx, 2 represents multi agent tx
};
...
export class AptosWallet extends BaseWallet {
    ...
    checkPrivateKey(privateKey: string) {
        ...
        const keyBytes = base.fromHex(
            stripPrivateKeyPrefix(privateKey).toLowerCase()
        );
    ...
    getNewAddress(param: NewAddressParams): Promise<any> {
        try {
            ...
            const account = AptosAccount.fromPrivateKey(
                HexString.ensure(stripPrivateKeyPrefix(param.privateKey))
            );
        ...
        try {
            ...
            switch (simulateParam.type) {
                case 'simulate_transfer':
                    ...
                    const tfPayload = transferPayload(
                        data.recipientAddress,
                        BigInt(data.amount)
                    );
                    const tfRawTxn = createRawTransaction(
                    ...
                    tx = generateBCSSimulateTransactionWithPublicKey(
                        publicKey,
                        tfRawTxn
                    );
                    break;
                case 'simulate_token_transfer':
                    const tokenData =
                        simulateParam.data as AptosTokenTransferParam;
                ...
                case 'simulate_dapp':
                    ...
                    if (dappData.type == 3) {
                        ...
                        return generateTransactionPayload(
                            parseInputScriptDataParam(paramData)
                        ).then((payload) => {
                            return createRawTransactionV2(
                                ...
                                BigInt(baseParam.gasUnitPrice),
                                BigInt(baseParam.expirationTimestampSecs),
                                baseParam.withFeePayer,
                                baseParam.secondarySignerAddresses
                            ).then((rawTxn) => {
                                let res =
                                    generateSignedTransactionForSimulation({
                                        transaction: rawTxn,
                        ...
                        const deserializer = new Deserializer(
                            base.fromHex(dappData.data)
                        );
                        const payload =
                            TransactionPayloadV2.deserialize(deserializer);
                        return createRawTransactionV2(
                            HexString.fromBuffer(sender.toUint8Array()),
                            payload,
                            BigInt(baseParam.sequenceNumber),
                        ...
                        return createSimulateRawTransactionByABIV2(
                            sender,
                            pubKey,
                            BigInt(baseParam.sequenceNumber),
                            baseParam.chainId,
                ...
                case 'simulate_fungible_asset_transfer':
                    const faData =
                        simulateParam.data as AptosFungibleTokenTransferParam;
                    ...
                    const senderAddress = new AccountAddress(
                        sender.toUint8Array()
                    );
                    const res = fungibleAsset
                        ...
                        .then((transferFungibleAssetRawTransaction) => {
                            const signedTx =
                                generateSignedTransactionForSimulation({
                                    transaction:
                                        transferFungibleAssetRawTransaction,
                                    signerPublicKey: pubKey,
                                });
                            return base.toHex(signedTx);
                        });
    ...
    async signTransaction(param: SignTxParams): Promise<any> {
        ...
        try {
            ...
            // compatible v2
            const privateKey = HexString.ensure(
                stripPrivateKeyPrefix(param.privateKey)
            ).toString();
            ...
            switch (tp) {
                ...
                case 'tokenTransfer': {
                    ...
                    if (baseParam.withFeePayer) {
                        ...
                        return createRawTransactionV2(
                            sender,
                            TransactionPayloadV2.deserialize(
                                Deserializer.fromHex(ser.getBytes())
                            ),
                            BigInt(baseParam.sequenceNumber),
                            baseParam.chainId,
                            BigInt(baseParam.maxGasAmount),
                ...
                case 'signAsFeePayer': {
                    ...
                    let anyTx;
                    if (data.type == '2') {
                        anyTx = MultiAgentTransaction.deserialize(
                            Deserializer.fromHex(data.rawTxn)
                        );
                    } else {
                        anyTx = SimpleTransaction.deserialize(
                            Deserializer.fromHex(data.rawTxn)
                        );
                    }
                    anyTx.feePayerAddress = senderAddress;
                    const feePayerSig =
                        senderAccount.signTransactionWithAuthenticator(anyTx);
                    return Promise.resolve({
                ...
                case 'dapp': {
                    ...
                    if (data.type == 3) {
                        ...
                        return createRawTransactionByABIV2(
                            senderAccount,
                            BigInt(baseParam.sequenceNumber),
                            baseParam.chainId,
                            BigInt(baseParam.maxGasAmount),
                ...
                case 'offerNftObject': {
                    ...
                    const payload = offerNFTTokenPayloadObject(
                        HexString.ensure(data.nftObject),
                        HexString.ensure(data.receiver),
                        BigInt(data.amount)
                    );
                    const rawTxn = createRawTransaction(
                        sender,
                ...
                case 'offerNft': {
                    ...
                    const payload = offerNFTTokenPayload(
                        HexString.ensure(data.receiver),
                        HexString.ensure(data.creator),
                        data.collectionName,
                        data.tokenName,
                        BigInt(data.version),
                        BigInt(data.amount)
                ...
                case 'claimNft': {
                    ...
                    const payload = claimNFTTokenPayload(
                        HexString.ensure(data.sender),
                        HexString.ensure(data.creator),
                        data.collectionName,
                        data.tokenName,
                        BigInt(data.version)
                ...
                case 'simple_transaction': {
                    ...
                    const moveModule =
                        data.moveModule == undefined ? m : data.moveModule;
                    const aptosConfig = new AptosConfig({
                        network: network,
                    ...
                    if (data.signAsFeePayer) {
                        ...
                        const rawTx = new SimpleTransaction(
                            rawTransaction,
                            AccountAddress.ZERO
                        );
                        // sponsor signs
                    ...
                    const functionArguments =
                        data.functionArguments === undefined
                            ? [data.recipientAddress, data.amount]
                            : data.functionArguments;
                    const res = transaction.build
                        .simple({
    ...
    validAddress(param: ValidAddressParams): Promise<any> {
        ...
        try {
            if (
                param.address.match(/^0x[0-9A-Fa-f]{62,64}$/) ||
                param.address.match(/^[0-9A-Fa-f]{64}$/)
            ) {
                isValid = true;
            } else {
                isValid = false;
    ...
    async signMessage(param: SignTxParams): Promise<string> {
        ...
        try {
            ...
            let data = await client.signMessage(
                message,
                stripPrivateKeyPrefix(param.privateKey)
            );
    ...
    async signCommonMsg(params: SignCommonMsgParams): Promise<any> {
        ...
        return super.signCommonMsg({
            privateKey: stripPrivateKeyPrefix(params.privateKey),
            privateKeyHex: stripPrivateKeyPrefix(params.privateKey),
            message: params.message,
            signType: SignType.ED25519,
        });
    }

    async signMessageByPayload(
        param: SignMessageByPayloadParams
    ): Promise<any> {
        try {
            ...
            if (messagePayload?.address) {
                const account = new AptosAccount(
                    base.fromHex(stripPrivateKeyPrefix(param.privateKey))
                );
            ...
            if (messagePayload?.application) {
                application =
                    messagePayload.dAppDomain == undefined
                        ? ''
                        : messagePayload.dAppDomain;
                fullMessage = fullMessage.concat(
                    '\napplication: ',
                    application
                );
            ...
            if (messagePayload.chainId) {
                chainId =
                    messagePayload.chain_id == undefined
                        ? 0
                        : messagePayload.chain_id;
                fullMessage = fullMessage.concat(
                    '\nchainId: ',
                    chainId.toString()
                );
    ...
    async validSignedTransaction(
        ...
        try {
            const skipCheckSign = param.data
                ...
                : undefined;
            const ret = client.validSignedTransaction(
                param.tx,
                skipCheckSign || false
            );
