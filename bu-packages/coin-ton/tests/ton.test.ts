import { base, SignTxError, ValidAddressError } from '@okxweb3/coin-base';
import {
    generateMnemonic,
    isPasswordNeeded,
    mnemonicToKeyPair,
...
describe('ton', () => {
    ...
    test('calcTxHash', async () => {
        const param = {
            data: '',
        };
        const msgHash = await tonWallet.calcTxHash(param);
        expect(msgHash).toBe(
            '5eea127ebda5a0be6b037fb57f6f6b9511d870f108442a2f8b0a1c41fa39f4e6'
        );
    });

    test('calcTxHash internal message - should fail', async () => {
        const param = {
            data: 'te6cckEBAQEAaAAAy0gB8Cn1fHohxPZIFSRZipo2pB5XOKEVVx6X/5rkzVNB09cAPgU+r49EOJ7JAqSLMVNG1IPK5xQiquPS//NcmapoOnrQF9eEAAAAAAAAAAAAANHzPQ4AAAAAOjK5uhA2srm5sLOywHgCcBg=',
        };
        await expect(tonWallet.calcTxHash(param)).rejects.toBeDefined();
    });
    ...
    test('signMultiTransaction', async () => {
            ...
            data: {
                messages: [
                    ...
                    },
                    {
                        ...
                        stateInit: '', //base64
                        isBase64Payload: true,
                        extraFlags: '3', // with bounce response flagsj
                    },
                ],
        ...
        const expected =
            '';
...
describe('TON address validation in transactions', () => {
    const validPrivateKey =
        'fc81e6f42150458f53d8c42551a8ab91978a55d0e22b1fd890b85139086b93f8';
    const timeoutAtSeconds = Math.floor(Date.now() / 1e3) + 600;

    describe('Valid TON addresses should work', () => {
        const validTonAddresses = [
            'UQC6QJ31Bv_hjmsoaUjRmpZYqj9NXbBbvufCNycnc0gjReqR', // From existing tests
            'EQA3_JIJKDC0qauDUEQe2KjQj1iLwQRtrEREzmfDxbCKw9Kr', // From existing tests
            'EQC6QJ31Bv_hjmsoaUjRmpZYqj9NXbBbvufCNycnc0gjRbdU', // Bounceable version
            '0:ba409df506ffe18e6b286948d19a9658aa3f4d5db05bbee7c237272773482345', // Raw format
        ];

        validTonAddresses.forEach((address, index) => {
            test(`should sign transfer to valid TON address: ${address.substring(
                0,
                20
            )}...`, async () => {
                const param = {
        ...
        test('should sign jetton transfer to valid TON address', async () => {
            const param = {
                privateKey: validPrivateKey,
                data: {
    ...
    describe('Invalid addresses should be rejected', () => {
        const invalidAddresses = [
            ...
            {
                address: '0x742d35Cc6634C0532925a3b8D6C9E0C8b3a8C0E3',
            ...
            {
                address: 'rnyyyTfmc4MLRXqn7fhJW51fxttPp4djHw',
                name: 'XRP address',
            },
            { address: '', name: 'Empty string' },
            { address: 'invalid_ton_address', name: 'Invalid format' },
            {
                address: 'UQC6QJ31Bv_hjmsoaUjRmpZYqj9NXbBbvufCNycnc0gj',
                name: 'TON address too short',
            },
            {
                address: 'UQC6QJ31Bv_hjmsoaUjRmpZYqj9NXbBbvufCNycnc0gjReqRXXXX',
                name: 'TON address too long',
            },
        ];

        invalidAddresses.forEach((testCase) => {
            test(`should reject transfer to ${testCase.name}: ${testCase.address}`, async () => {
                const param = {
                    privateKey: validPrivateKey,
                    data: {
                        type: 'transfer' as const,
                        ...
                        seqno: 1,
                        toIsInit: true,
                        memo: `test invalid ${testCase.name}`,
                        expireAt: timeoutAtSeconds,
                        sendMode: 1,
                    },
                };

                await expect(tonWallet.signTransaction(param)).rejects.toBe(
                    ValidAddressError
                );
            });
        });
        ...
        invalidAddresses.forEach((testCase) => {
            test(`should reject jetton transfer to ${testCase.name}: ${testCase.address}`, async () => {
                const param = {
                    privateKey: validPrivateKey,
                    data: {
                        type: 'jettonTransfer' as const,
                        ...
                        seqno: 1,
                        toIsInit: false,
                        memo: `invalid jetton to ${testCase.name}`,
                        decimal: 2,
                        amount: '100',
                        messageAttachedTons: '50000000',
                        invokeNotificationFee: '1',
    ...
    describe('Multi-transaction address validation', () => {
        test('should reject multi-transaction with invalid address in messages', async () => {
            const param = {
                privateKey: validPrivateKey,
                data: {
                    messages: [
            ...
            await expect(tonWallet.signMultiTransaction(param)).rejects.toBe(
                SignTxError
            );
        });

        test('should reject multi-transaction with TRON address in messages', async () => {
            const param = {
                privateKey: validPrivateKey,
                data: {
                    messages: [
                        {
                            address: 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t', // TRON address
                            amount: '100000000',
                            payload: 'TRON address test',
                            stateInit: '',
                            isBase64Payload: false,
                        },
                    ],
        ...
        test('should reject multi-transaction with empty address in messages', async () => {
            const param = {
                privateKey: validPrivateKey,
                data: {
                    messages: [
        ...
        test('should accept multi-transaction with all valid TON addresses', async () => {
            const param = {
                privateKey: validPrivateKey,
                data: {
                    messages: [
    ...
    describe('Address validation edge cases', () => {
        test('should validate addresses using validAddress method', async () => {
            ...
            for (const address of validAddresses) {
                const result = await tonWallet.validAddress({ address });
                expect(result.isValid).toBe(true);
            }
            ...
            const invalidAddresses = [
                ...
                '0x742d35Cc6634C0532925a3b8D6C9E0C8b3a8C0E3', // Ethereum
