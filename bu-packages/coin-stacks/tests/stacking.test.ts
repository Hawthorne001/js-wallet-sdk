...
describe('decodeBtcAddress', () => {
    ...
    test('should throw InvalidAddressError for malformed segwit address', () => {
        expect(() => decodeBtcAddress('bc1invalid')).toThrow(
            InvalidAddressError
        );
    });
});
...
describe('extractPoxAddressFromClarityValue', () => {
    test('should extract pox address from valid tuple', () => {
        const versionBuffer = bufferCV(
            new Uint8Array([PoXAddressVersion.P2PKH])
        );
...
describe('getErrorString', () => {
    test('should return correct error strings for all stacking errors', () => {
        ...
        expect(
            getErrorString(StackingErrors.ERR_STACKING_CORRUPTED_STATE)
        ).toBe('Stacking state is corrupted');
        expect(
            getErrorString(StackingErrors.ERR_STACKING_INSUFFICIENT_FUNDS)
        ).toBe('Insufficient funds');
        expect(
            getErrorString(StackingErrors.ERR_STACKING_INVALID_LOCK_PERIOD)
        ).toBe('Invalid lock period');
        expect(
            getErrorString(StackingErrors.ERR_STACKING_ALREADY_STACKED)
        ).toBe('Account already stacked. Concurrent stacking not allowed.');
        expect(
            getErrorString(StackingErrors.ERR_STACKING_NO_SUCH_PRINCIPAL)
        ).toBe('Principal does not exist');
        ...
        expect(
            getErrorString(StackingErrors.ERR_STACKING_PERMISSION_DENIED)
        ).toBe('Permission denied');
        expect(
            getErrorString(StackingErrors.ERR_STACKING_THRESHOLD_NOT_MET)
        ).toBe('Stacking threshold not met');
        expect(
            getErrorString(StackingErrors.ERR_STACKING_POX_ADDRESS_IN_USE)
        ).toBe('PoX address already in use');
        expect(
            getErrorString(StackingErrors.ERR_STACKING_INVALID_POX_ADDRESS)
        ).toBe('Invalid PoX address');
        expect(
            getErrorString(StackingErrors.ERR_STACKING_ALREADY_REJECTED)
        ).toBe('Stacking already rejected');
        ...
        expect(
            getErrorString(StackingErrors.ERR_STACKING_ALREADY_DELEGATED)
        ).toBe('Already delegated');
        expect(
            getErrorString(StackingErrors.ERR_DELEGATION_EXPIRES_DURING_LOCK)
        ).toBe('Delegation expires during lock period');
        expect(
            getErrorString(StackingErrors.ERR_DELEGATION_TOO_MUCH_LOCKED)
        ).toBe('Delegation too much locked');
        expect(
            getErrorString(StackingErrors.ERR_DELEGATION_POX_ADDR_REQUIRED)
        ).toBe('PoX address required for delegation');
        expect(
            getErrorString(StackingErrors.ERR_INVALID_START_BURN_HEIGHT)
        ).toBe('Invalid start burn height');
        ...
        expect(
            getErrorString(StackingErrors.ERR_STACK_INCREASE_NOT_LOCKED)
        ).toBe('Stacker must be currently locked');
        expect(
            getErrorString(StackingErrors.ERR_DELEGATION_NO_REWARD_SLOT)
        ).toBe('Invalid reward-cycle and reward-cycle-index');
        expect(
...
describe('poxAddressToBtcAddress', () => {
    test('should convert version and hashBytes to mainnet P2PKH address', () => {
        ...
        const hashBytes = new Uint8Array([
            0x62, 0xe9, 0x07, 0xb1, 0x5c, 0xbf, 0x27, 0xd5, 0x42, 0x53, 0x99,
            0xeb, 0xf6, 0xf0, 0xfb, 0x50, 0xeb, 0xb8, 0x8f, 0x18,
        ]);
    ...
    test('should throw error for unexpected address version', () => {
        ...
        expect(() =>
            poxAddressToBtcAddress(version, hashBytes, 'mainnet')
        ).toThrow('Unexpected address version: 99');
    });
...
describe('unwrap', () => {
    ...
    test('should throw error for non-optional value', () => {
        ...
        expect(() => unwrap(nonOptional)).toThrow(
...
describe('Edge Cases and Error Conditions', () => {
    test('should handle malformed bech32 addresses', () => {
        ...
        expect(() => decodeBtcAddress('tb1invalid')).toThrow(
            InvalidAddressError
        );
    });
