...
describe('Encryption Module', () => {
    ...
    describe('keys', () => {
        ...
        test('isValidPrivateKey should validate private keys correctly', () => {
            ...
            expect(
                isValidPrivateKey(hexToBytes(validPrivateKeyWithout0x))
            ).toBe(true);
        ...
        test('getPublicKeyFromPrivate should derive public key', () => {
            ...
            const publicKey2 = getPublicKeyFromPrivate(
            ...
            expect(
                publicKey1.startsWith('02') || publicKey1.startsWith('03')
            ).toBe(true); // Compressed format
        });
        ...
        test('base58CheckDecode should throw on invalid checksum', () => {
            ...
            const invalidChecksumAddress =
                validBitcoinAddress.slice(0, -1) + 'X';
    ...
    describe('sha2Hash', () => {
        ...
        test('NodeCryptoSha2Hash should digest data', async () => {
            ...
            const mockCreateHash = jest.fn().mockReturnValue({
                ...
                digest: jest
                    .fn()
                    .mockReturnValue(new Uint8Array(32).fill(0x12)),
            });
        ...
        test('NodeCryptoSha2Hash should handle sha512', async () => {
            const mockCreateHash = jest.fn().mockReturnValue({
                ...
                digest: jest
                    .fn()
                    .mockReturnValue(new Uint8Array(64).fill(0x34)),
            });
    ...
    describe('ec', () => {
        ...
        test('getBytesFromBN should convert bigint to bytes', () => {
            ...
            expect(getBytesFromBN(BigInt(0))).toEqual(
    ...
    describe('varuint', () => {
        ...
        test('encode/decode roundtrip should work', () => {
            const testNumbers = [
                0, 1, 252, 253, 254, 255, 256, 65535, 65536, 4294967295,
                4294967296,
            ];
    ...
    describe('base64', () => {
        describe('base64ByteLength', () => {
            ...
            test('should throw on invalid base64 length', () => {
                expect(() => base64ByteLength('Q')).toThrow(
                    'Invalid string. Length must be a multiple of 4'
                );
                expect(() => base64ByteLength('QU')).toThrow(
                    'Invalid string. Length must be a multiple of 4'
                );
                expect(() => base64ByteLength('QUJ')).toThrow(
                    'Invalid string. Length must be a multiple of 4'
                );
            });
        });
        ...
        describe('base64ToByteArray', () => {
            ...
            test('should handle padding correctly', () => {
                ...
                expect(base64ToByteArray('QUI=')).toEqual(
                    new Uint8Array([65, 66])
                ); // 'AB'
                expect(base64ToByteArray('QUJD')).toEqual(
            ...
            test('should decode standard test vectors', () => {
                ...
                expect(base64ToByteArray('Zg==')).toEqual(
                    new Uint8Array([102])
                ); // 'f'
                expect(base64ToByteArray('Zm8=')).toEqual(
                    new Uint8Array([102, 111])
                ); // 'fo'
                expect(base64ToByteArray('Zm9v')).toEqual(
                    new Uint8Array([102, 111, 111])
                ); // 'foo'
                expect(base64ToByteArray('Zm9vYg==')).toEqual(
                    new Uint8Array([102, 111, 111, 98])
                ); // 'foob'
                expect(base64ToByteArray('Zm9vYmE=')).toEqual(
                    new Uint8Array([102, 111, 111, 98, 97])
                ); // 'fooba'
                expect(base64ToByteArray('Zm9vYmFy')).toEqual(
        ...
        describe('base64FromByteArray', () => {
            ...
            test('should encode multiple bytes', () => {
                const result = base64FromByteArray(
                    new Uint8Array([65, 66, 67])
                ); // 'ABC'
            ...
            test('should handle different padding scenarios', () => {
                ...
                expect(base64FromByteArray(new Uint8Array([65, 66]))).toBe(
                    'QUI='
                ); // 2 bytes -> 1 padding
                expect(base64FromByteArray(new Uint8Array([65, 66, 67]))).toBe(
                    'QUJD'
                ); // 3 bytes -> no padding
            });
            ...
            test('should encode standard test vectors', () => {
                ...
                expect(base64FromByteArray(new Uint8Array([102, 111]))).toBe(
                    'Zm8='
                ); // 'fo'
                expect(
                    base64FromByteArray(new Uint8Array([102, 111, 111]))
                ).toBe('Zm9v'); // 'foo'
                expect(
                    base64FromByteArray(new Uint8Array([102, 111, 111, 98]))
                ).toBe('Zm9vYg=='); // 'foob'
                expect(
                    base64FromByteArray(new Uint8Array([102, 111, 111, 98, 97]))
                ).toBe('Zm9vYmE='); // 'fooba'
                expect(
                    base64FromByteArray(
                        new Uint8Array([102, 111, 111, 98, 97, 114])
                    )
                ).toBe('Zm9vYmFy'); // 'foobar'
            });
            ...
            test('byteLength should match actual decoded length', () => {
                ...
                for (const base64String of testCases) {
                    if (base64String.length % 4 === 0) {
                        // Only test valid base64 strings
