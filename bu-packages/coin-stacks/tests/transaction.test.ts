...
describe('Authorization Tests', () => {
    ...
    test('createSponsoredAuth', () => {
        ...
        if (auth.authType === AuthType.Sponsored) {
            expect(
                (auth as SponsoredAuthorization).sponsorSpendingCondition
            ).toBe(sponsorSpendingCondition);
        }
    });
    ...
    test('setSponsor', () => {
        ...
        if (updatedAuth.authType === AuthType.Sponsored) {
            expect(
                (updatedAuth as SponsoredAuthorization).sponsorSpendingCondition
                    .nonce
            ).toBe(BigInt(3));
            expect(
                (updatedAuth as SponsoredAuthorization).sponsorSpendingCondition
...
describe('Post Conditions Tests', () => {
    ...
    test('createContractPrincipal', () => {
        const principal = createContractPrincipal(
            contractAddress,
            contractName
        );
...
describe('Multi-signature Advanced Tests', () => {
    ...
    test('sponsored transaction with different spending conditions', () => {
        ...
        if (sponsoredAuth.authType === AuthType.Sponsored) {
            expect(
                (sponsoredAuth as SponsoredAuthorization)
                    .sponsorSpendingCondition.fee
            ).toBe(BigInt(1000));
        }
    });
});
...
describe('Contract ABI Tests', () => {
    ...
    describe('Type Guards Tests', () => {
        test('isClarityAbiPrimitive', () => {
            ...
            expect(isClarityAbiPrimitive({ buffer: { length: 32 } })).toBe(
                false
            );
        ...
        test('isClarityAbiStringAscii', () => {
            expect(
                isClarityAbiStringAscii({ 'string-ascii': { length: 32 } })
            ).toBe(true);
            expect(
                isClarityAbiStringAscii({ 'string-ascii': { length: 1 } })
            ).toBe(true);

            expect(isClarityAbiStringAscii('uint128')).toBe(false);
            expect(
                isClarityAbiStringAscii({ 'string-utf8': { length: 32 } })
            ).toBe(false);
        });

        test('isClarityAbiStringUtf8', () => {
            expect(
                isClarityAbiStringUtf8({ 'string-utf8': { length: 32 } })
            ).toBe(true);
            expect(
                isClarityAbiStringUtf8({ 'string-utf8': { length: 1 } })
            ).toBe(true);

            expect(isClarityAbiStringUtf8('uint128')).toBe(false);
            expect(
                isClarityAbiStringUtf8({ 'string-ascii': { length: 32 } })
            ).toBe(false);
        });
        ...
        test('isClarityAbiTuple', () => {
            ...
            expect(
                isClarityAbiTuple({ list: { type: 'uint128', length: 10 } })
            ).toBe(false);
        });

        test('isClarityAbiList', () => {
            expect(
                isClarityAbiList({ list: { type: 'uint128', length: 10 } })
            ).toBe(true);
            expect(
                isClarityAbiList({
    ...
    describe('getTypeString Tests', () => {
        ...
        test('optional types', () => {
            expect(getTypeString({ optional: 'uint128' })).toBe(
                '(optional uint)'
            );
            expect(
                getTypeString({ optional: { buffer: { length: 32 } } })
            ).toBe('(optional (buff 32))');
        });
        ...
        test('list types', () => {
            expect(
                getTypeString({ list: { type: 'uint128', length: 10 } })
            ).toBe('(list 10 uint)');
            ...
            expect(getTypeString(complexListType)).toBe(
                '(list 5 (tuple (id uint)))'
            );
        });
    ...
    describe('parseToCV Tests', () => {
        ...
        test('bool parsing', () => {
            ...
            expect(() => parseToCV('invalid', 'bool')).toThrow(
                'Invalid bool value'
            );
        });
    ...
    describe('abiFunctionToString Tests', () => {
        ...
        test('private function', () => {
            ...
            const result = abiFunctionToString(func);
            expect(result).toBe(
                '(define-private (internal-helper (value uint)))'
            );
        });
    ...
    describe('validateContractCall Tests', () => {
        ...
        test('valid contract call', () => {
            ...
            expect(() =>
                validateContractCall(payload, sampleAbi)
            ).not.toThrow();
...
describe('Post Condition Builder (Pc) Tests', () => {
    ...
    describe('PartialPcFtWithCode STX post conditions', () => {
        ...
        test('ustx() with different condition codes', () => {
            ...
            expect(lteCondition.conditionCode).toBe(
                FungibleConditionCode.LessEqual
            );
            ...
            expect(gtCondition.conditionCode).toBe(
    ...
    describe('End-to-end fluent API tests', () => {
        ...
        test('Complex scenario with multiple condition types', () => {
            ...
            const lessEqualCondition = principal(address)
                .willSendLte(200)
                .ustx();
            const lessCondition = principal(address).willSendLt(300).ustx();
            const greaterEqualCondition = principal(address)
                .willSendGte(400)
                .ustx();
            const greaterCondition = principal(address).willSendGt(500).ustx();

            expect(equalCondition.conditionCode).toBe(
                FungibleConditionCode.Equal
            );
            expect(lessEqualCondition.conditionCode).toBe(
                FungibleConditionCode.LessEqual
            );
            expect(lessCondition.conditionCode).toBe(
            ...
            const sendsNftCondition = principal(address)
                .willSendAsset()
                .nft(
                    'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.nft::token',
                    uintCV(1)
                );
...
describe('Signature Module Tests', () => {
    ...
    describe('deserializeTransactionAuthField tests', () => {
        ...
        test('deserializes compressed signature auth field', () => {
            ...
            expect(authField.type).toBe(StacksMessageType.TransactionAuthField);
            expect(authField.pubKeyEncoding).toBe(PubKeyEncoding.Compressed);
            expect(authField.contents.type).toBe(
                StacksMessageType.MessageSignature
            );
        });
    ...
    describe('Round-trip serialization tests', () => {
        test('public key compressed round-trip', () => {
            ...
            expect(deserialized.pubKeyEncoding).toBe(
                originalField.pubKeyEncoding
            );
            expect(deserialized.contents.type).toBe(
                originalField.contents.type
            );
        });
...
describe('TransactionSigner Tests', () => {
    ...
    describe('getTxInComplete tests', () => {
        test('returns cloned incomplete transaction', () => {
            ...
            expect(incompleteTransaction.version).toBe(
                signer.transaction.version
            );
        });
    });
    ...
    describe('signStructuredData tests', () => {
        ...
        test('signs structured data with string message', () => {
            ...
            expect(signature.type).toBe(
                StacksMessageType.StructuredDataSignature
            );
            expect(signature.data).toBeDefined();
...
describe('Clarity Value Conversion Tests', () => {
    describe('cvToString', () => {
        ...
        test('should convert standard principal to string', () => {
            const value = standardPrincipalCV(
                'SP2P58SJY1XH6GX4W3YGEPZ2058DD3JHBPJ8W843Q'
            );
            expect(cvToString(value)).toBe(
                'SP2P58SJY1XH6GX4W3YGEPZ2058DD3JHBPJ8W843Q'
            );
        });

        test('should convert contract principal to string', () => {
            const value = contractPrincipalCV(
                'SP2P58SJY1XH6GX4W3YGEPZ2058DD3JHBPJ8W843Q',
                'test-contract'
            );
            expect(cvToString(value)).toBe(
        ...
        test('should convert tuple to string', () => {
            ...
            expect(cvToString(value)).toBe(
                '(tuple (name "test") (value u42) (active true))'
            );
        });
        ...
        test('should handle nested structures with tryAscii encoding', () => {
            ...
            expect(cvToString(nestedValue, 'tryAscii')).toBe(
                '(tuple (message "Hello") (count u5))'
            );
        });
    });
    ...
    describe('getCVTypeString', () => {
        ...
        test('should return response error type for response err values', () => {
            ...
            const value = responseErrorCV(innerValue);
            expect(getCVTypeString(value)).toBe(
                '(response UnknownType (string-ascii 5))'
            );
        });
        ...
        test('should return principal for principal values', () => {
            const standardValue = standardPrincipalCV(
                'SP2P58SJY1XH6GX4W3YGEPZ2058DD3JHBPJ8W843Q'
            );
            expect(getCVTypeString(standardValue)).toBe('principal');

            const contractValue = contractPrincipalCV(
                'SP2P58SJY1XH6GX4W3YGEPZ2058DD3JHBPJ8W843Q',
                'test'
            );
        ...
        test('should return tuple type with field types', () => {
            ...
            expect(result).toBe(
                '(tuple (name (string-ascii 4)) (value uint) (active bool))'
            );
        });
        ...
        test('should handle complex nested structures', () => {
            ...
            expect(result).toBe(
                '(tuple (data (list 2 uint)) (response (response (string-ascii 7) UnknownType)) (optional (optional bool)))'
            );
        });
    });
});
...
describe('Direct Function Address Validation', () => {
    describe('transfer function validation', () => {
        const validPrivateKey =
            '33c4ad314d494632a36c27f9ac819e8d2986c0e26ad63052879f631a417c8adf';

        test('should reject empty toAddress', () => {
            expect(() =>
                transfer(validPrivateKey, '', 1000000, 'test memo', 1, 1000)
            ).toThrow('Invalid recipient address');
        });

        test('should reject whitespace-only toAddress', () => {
            expect(() =>
                transfer(
                    validPrivateKey,
                    '   \t\n   ',
        ...
        test('should accept valid toAddress', () => {
            try {
                transfer(
                    validPrivateKey,
                    'SP2XYBM8MD5T50WAMQ86E8HKR85BAEKBECNE1HHVY',
                    1000000,
                    'test memo',
                    1,
                    1000
                ...
                // If it fails, it should not be due to address validation
    ...
    describe('tokenTransfer function validation', () => {
        const validPrivateKey =
            '33c4ad314d494632a36c27f9ac819e8d2986c0e26ad63052879f631a417c8adf';

        test('should reject empty toAddress', async () => {
            await expect(
                tokenTransfer(
                    validPrivateKey,
                    'SP2XYBM8MD5T50WAMQ86E8HKR85BAEKBECNE1HHVY',
        ...
        test('should reject invalid toAddress', async () => {
            await expect(
                tokenTransfer(
                    validPrivateKey,
                    'SP2XYBM8MD5T50WAMQ86E8HKR85BAEKBECNE1HHVY',
