...
function serializeStruct(
    ...
    if (typeof data !== 'object') {
        throw new Error(
            'expected object containing data: ' + JSON.stringify(data)
        );
    }
    if (this.base) {
    ...
    const result: Map<string, Type> = new Map(
        Object.entries({
            bool: createType({
                ...
                serialize: (buffer: SerialBuffer, data: boolean) => {
                    if (
                        ...
                        throw new Error(
                            'Expected boolean or number equal to 1 or 0'
                        );
                    }
            ...
            int64: createType({
                ...
                serialize: (buffer: SerialBuffer, data: string | number) => {
                    buffer.pushArray(
                        numeric.signedDecimalToBinary(8, '' + data)
                    );
                },
                deserialize: (buffer: SerialBuffer) => {
                    return numeric.signedBinaryToDecimal(
                        buffer.getUint8Array(8)
                    );
                },
            }),
            int32: createType({
            ...
            int128: createType({
                ...
                serialize: (buffer: SerialBuffer, data: string) => {
                    buffer.pushArray(
                        numeric.signedDecimalToBinary(16, '' + data)
                    );
                },
                deserialize: (buffer: SerialBuffer) => {
                    return numeric.signedBinaryToDecimal(
                        buffer.getUint8Array(16)
                    );
                },
            }),
            float32: createType({
