/**
 * The following methods are based on `eosjs`, thanks for their work
 * https://github.com/EOSIO/eosjs/blob/master/src/eosjs-numeric.ts
 */
/**
 * @module Numeric
 */
...
export const stringToPublicKey = (s: string): Key => {
    if (s.substr(0, 3) === 'EOS') {
        ...
        const digest = base.ripemd160(key.data);
        if (
...
export const privateKeyToLegacyString = (key: Key): string => {
    if (key.type === KeyType.k1 && key.data.length === privateKeyDataSize) {
        ...
        const digest = base.doubleSha256(Uint8Array.from(whole));
    ...
    const signBuf = Buffer.concat([
        ...
        Buffer.from(
            serializedContextFreeData
                ? new Uint8Array(base.sha256(serializedContextFreeData))
                : new Uint8Array(32)
        ),
    ]);
    return base.sha256(signBuf);
};

export function signatureToElliptic(signature: Uint8Array) {
    const lengthOfR = 32;
    const lengthOfS = 32;
    const signatureBytes = signature.slice(1, lengthOfR + lengthOfS + 1);

    let ellipticRecoveryBitField;
    ellipticRecoveryBitField = signature[0] - 27;
    if (ellipticRecoveryBitField > 3) {
        ellipticRecoveryBitField -= 4;
    }
    const recoveryParam = ellipticRecoveryBitField & 3;
    return { signatureBytes, recoveryParam };
}
