...
export class TxBuilder {
    ...
    public getTransactionAbiMap(
        ...
        const accounts: string[] = actions.map(
            (action: Action): string => action.account
        );
    ...
    public serializeTransactionExtensions(
        ...
        if (resource_payer) {
            ...
            types
                .get('resource_payer')!
                .serialize(extensionBuffer, resource_payer);
            transaction_extensions = [
                ...transaction_extensions,
                [1, arrayToHex(extensionBuffer.asUint8Array())],
            ];
        }
    ...
    public serializeContextFreeData(
    ...
    public build(
        ...
        const refBlockInfo = {
            block_num: config.refBlockNumber,
            id: config.refBlockId,
            timestamp: config.refBlockTimestamp,
        };
        ...
        const transactionAbiMap = this.getTransactionAbiMap(
            transaction,
            abiMap
        );
        transaction.transaction_extensions =
            this.serializeTransactionExtensions(
                transaction,
                config.resource_payer
            );
        transaction.context_free_actions = this.serializeActions(
            transaction.context_free_actions || [],
            transactionAbiMap
        );
        transaction.actions = this.serializeActions(
            transaction.actions,
            transactionAbiMap
        );
        ...
        if (config.compression) {
            ...
            return {
                ...
                packed_context_free_data: arrayToHex(
                    compressedSerializedContextFreeData || new Uint8Array(0)
                ),
                packed_trx: arrayToHex(
                    compressedSerializedTransaction || new Uint8Array(0)
                ),
            };
        } else {
            return {
    ...
    public sign(
        ...
        for (const key of privateKeys) {
            ...
            let signature: Key;
            do {
                const rawSign = ec.sign(
                    Array.from(digest),
                    Buffer.from(privateKey),
                    { canonical: true, pers: [++tries] }
                );
                ...
                const d = new Uint8Array(
                    [eosioRecoveryParam].concat(...rawSign.toBytes())
                );
