...
describe('wax', () => {
    ...
    // Test validAddress method - now implemented
    test('validAddress works correctly', async () => {
        let wallet = new WaxWallet();

        // Test valid EOS account name
        const validResult = await wallet.validAddress({
            address: 'eosio.token',
        });
        expect(validResult.isValid).toBe(true);
        expect(validResult.address).toBe('eosio.token');

        // Test invalid EOS account name
        const invalidResult = await wallet.validAddress({
            address: 'invalid@name',
        });
        expect(invalidResult.isValid).toBe(false);
        expect(invalidResult.address).toBe('invalid@name');
    });
    ...
    test('signTransaction type 2 with requiredKeys', async () => {
        ...
        expect(res.serializedTransaction).toBe(
            txParams.data.serializedTransaction
        );
    });
