...
import {
    Height,
    MsgTransfer,
} from './lib/types/ibc/applications/transfer/v1/tx';
...
export function public2Address(
    publicKey: Uint8Array,
    useEthSecp256k1: boolean
): Uint8Array {
    return useEthSecp256k1
        ? base.keccak(publicKey.slice(1)).slice(-20)
        : base.hash160(publicKey);
}

export function private2Public(
...
export function private2Address(
    privateKey: Uint8Array,
    prefix: string,
    useEthSecp256k1: boolean
): string {
...
export function addressFromPublic(
    publicKeyHex: string,
    prefix: string = 'cosmos',
    useEthSecp256k1: boolean = false
): string {
    let address;
...
// ensures that address is valid bech32 format, has a prefix, and has data
export function validateAddressRelaxed(address: string) {
    try {
        const [prefix, data] = base.fromBech32(address);
        return !!(prefix && data.length > 0);
    } catch (e) {
        return false;
    }
}
...
export async function sendToken(
    ...
    const result = await signTx(
        ...
        fee,
        memo,
        Long.fromNumber(timeoutHeight || 0),
        {
...
export async function SignWithSignDocForINJ(
    privateKey: Uint8Array,
    message: string,
    useEthSecp256k1: boolean
) {
    ...
    const signDoc = makeSignDoc(
        base.fromHex(m.body),
        base.fromHex(m.authInfo),
        m.chainId,
        parseInt(m.accountNumber)
    );
    if (!privateKey) {
    ...
    const messageHash = useEthSecp256k1
        ? base.keccak256(signDocBytes)
        : base.sha256(signDocBytes);
    const { signature } = signUtil.secp256k1.sign(
...
export async function SignWithSignDocForINJWithTx(
    privateKey: Uint8Array,
    message: string,
    useEthSecp256k1: boolean
) {
    ...
    const txRaw = TxRaw.fromPartial({
        ...
        // signatures: [base.fromBase64(sig)],
        signatures: [signatureR1],
    });
    const tx = base.toBase64(TxRaw.encode(txRaw).finish());
    return Promise.resolve(jsonStringifyUniform({ signature: sig, tx: tx }));
}

export async function signWithStdSignDocForINJ(
    privateKey: Uint8Array,
    message: string,
    useEthSecp256k1: boolean
) {
...
export async function signWithStdSignDocForINJWithTx(
    privateKey: Uint8Array,
    message: string,
    useEthSecp256k1: boolean,
    wallet: CosmosWallet
    ...
    const tx = await sendAminoMessage(
        privateKey as Uint8Array,
        wallet.getPrefix(),
        message,
        wallet.getAminoConverters(),
        wallet.getExtraTypes(),
        wallet.supportEthSign(),
        wallet.pubKeyUrl()
    );
    const publicKey = private2Public(privateKey, true);
    const sig = encodeSecp256k1Signature(publicKey, signature, false);
...
export async function SignWithSignDoc(
    privateKey: Uint8Array,
    message: string,
    useEthSecp256k1: boolean
) {
...
export async function SignWithSignDocWithTx(
    privateKey: Uint8Array,
    message: string,
    useEthSecp256k1: boolean
) {
    ...
    const tx = base.toBase64(TxRaw.encode(txRaw).finish());
    return Promise.resolve(
        jsonStringifyUniform({ signature: signature, tx: tx })
    );
}

export async function signWithStdSignDoc(
    privateKey: Uint8Array,
    message: string,
    useEthSecp256k1: boolean
) {
...
export async function signWithStdSignDocWithTx(
    privateKey: Uint8Array,
    message: string,
    useEthSecp256k1: boolean,
    prefix: string,
    ...
    if (useEthSecp256k1) {
        const l = [Uint8Array.from(signature), Uint8Array.of(recovery)];
        const signatureR1 = Buffer.concat(l);
        const sig = encodeSecp256k1Signature(publicKey, signatureR1, true);
        return Promise.resolve(
            jsonStringifyUniform({ signature: sig, tx: tx })
        );
    } else {
        const sig = encodeSecp256k1Signature(publicKey, signature, false);
        return Promise.resolve(
...
export async function sendAminoMessage(
    privateKey: Uint8Array,
    prefix: string,
    // json
    data: string,
    ...
    const result = await signTx(
        ...
        m.fee,
        m.memo,
        Long.fromString(m.timeout_height || '0'),
        {
...
export function getMPCTransaction(
    raw: string,
    sig: string,
    publicKey: string,
    useEthSecp256k1?: boolean
    ...
    if (useEthSecp256k1) {
        ...
        const v = signUtil.secp256k1.getV(
            messageHash,
            base.toHex(r),
            base.toHex(s),
        ...
        sig = base.toHex(
            Buffer.concat([Uint8Array.from(signature), Uint8Array.of(v)])
        );
    }
...
export function getMPCSignedMessage(
    hash: string,
    sig: string,
    publicKey: string,
    useEthSecp256k1?: boolean
...
export function validSignedTransaction(
    ...
    if (
        !skipCheckSig &&
        !signUtil.secp256k1.verifyWithNoRecovery(hash, signature, publicKey.key)
    ) {
