...
export abstract class CosmosWallet extends BaseWallet {
    ...
    async checkPrivateKey(privateKeyHex: string) {
        ...
        return (
            privateKey.length == 32 && !privateKey.every((byte) => byte === 0)
        );
    }
    ...
    async signMessage0(param: SignTxParams): Promise<string> {
        try {
            ...
            if (message.type == 'amino') {
                const result = await signWithStdSignDoc(
                ...
                const result = await SignWithSignDoc(
                    privateKey as Buffer,
                    message.data,
                    ethSign
                );
    ...
    async signMessageWithTx(param: SignTxParams): Promise<any> {
        try {
            ...
            if (message.type == 'amino') {
                ...
                const result = await signWithStdSignDocWithTx(
                    privateKey as Buffer,
                    message.data,
                    ethSign,
                    prefix,
                ...
                const result = await SignWithSignDocWithTx(
                    privateKey as Buffer,
                    message.data,
                    ethSign
                );
    ...
    getAddressByPublicKey(param: GetAddressParams): Promise<string> {
        ...
        return Promise.resolve(
            addressFromPublic(param.publicKey, prefix, this.supportEthSign())
        );
    }
    ...
    async getMPCTransaction(param: MpcTransactionParam): Promise<string> {
        try {
            const ethSign = this.supportEthSign();
            const signedTx = getMPCTransaction(
                param.raw,
...
export class CommonCosmosWallet extends CosmosWallet {
    ...
    getExtraTypes(): ReadonlyArray<[string, GeneratedType]> | undefined {
        return [...OsmosisRegistry, ...CosmWasmRegistry];
    }
...
export class InjectiveWallet extends CosmosWallet {
    ...
    async signMessage0(param: SignTxParams): Promise<string> {
        try {
            ...
            if (message.type == 'amino') {
                const result = await signWithStdSignDocForINJ(
                    privateKey as Buffer,
                    message.data,
                    ethSign
                );
                ...
                const result = await SignWithSignDocForINJ(
                    privateKey as Buffer,
                    message.data,
                    ethSign
                );
    ...
    async signMessageWithTx(param: SignTxParams): Promise<any> {
        try {
            ...
            const message = param.data as SignMessageData;
            if (message.type == 'amino') {
                const result = await signWithStdSignDocForINJWithTx(
                    privateKey as Buffer,
                    message.data,
                    ethSign,
                    this
                ...
                const result = await SignWithSignDocForINJWithTx(
                    privateKey as Buffer,
                    message.data,
                    ethSign
                );
